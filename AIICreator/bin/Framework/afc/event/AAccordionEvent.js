...
{
	var acc = this.acomp, menu = selItem.menu, //$menu = $(selItem.menu), 
		isTrigger = false, timeoutVal = null;

	//각각의 이벤트 타입에 따라 컨텐츠 메뉴를 보여주고 숨기는 로직 처리
	menu.addEventListener(acc.option.showEvent, async function(e)
	{
		...
		if(acc.option.mouseOverEventDelay>0)
		{
			if(!isTrigger) //트리거로 이벤트를 발생시켰는지
			{
				timeoutVal = setTimeout(function()
				{
                    ...
                    _TinyDom.trigger(menu, 'mouseover')
				}, 
		...
		if(selItem.url && !selItem.contents.view)	//이미 로드되어져 있는 경우는 제외
		{
			...
			const aview = await AView.createView(selItem.contents, selItem.url, acc);
				
			acc._afterCreated(aview, selItem.contents.itemData);
		}
