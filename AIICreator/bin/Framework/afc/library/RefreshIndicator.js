...
RefreshIndicator.prototype.init = function(comp, scrollEle)
{
	...
	this.indicatorEle = document.createElement('div');
	this.indicatorEle.innerHTML = '\
	...
	Object.assign(this.indicatorEle.style,
	{
		position: 'absolute',
		margin: 'auto',
		left: 0,
	...
	this.controlDiv = this.indicatorEle.children[0];
	this.iconDiv = this.controlDiv.children[0];
	this.spinnerDiv = this.controlDiv.children[1];

	Object.assign(this.controlDiv.style, {
		position: 'absolute',
		width: '100%',
		height: '100%',
	...
	Object.assign(this.spinnerDiv.style, {
		'transform-origin': 'center center',
		animation: 'pull-to-refresh-material-rotate 2s linear infinite'
	});

	Object.assign(this.spinnerDiv.children[0].style, {
	...
	this.element.appendChild(this.indicatorEle);
	this.element.appendChild(this.modalBg);
	
	var thisObj = this,
		agrid = this.acomp,
		startX = 0, startY = 0,
		isTouchLeave = true,
		indiEle = this.indicatorEle;


	this.indicatorEle.addEventListener('transitionend', function()
	{
		if(['aborting', 'restoring'].indexOf(thisObj.state) < 0) return;

		Object.assign(indiEle.style, {
			opacity: 0,
			transition: ''
		});

		if (thisObj.type) {
			Object.assign(scrollEle.style, {
				transition: ''
			});
		}
...
RefreshIndicator.prototype._changeStyleByState = function(state)
{
	...
	if(state)
	{
		...
		{
			_TinyDom.show(this.iconDiv);
			_TinyDom.hide(this.spinnerDiv);
			_TinyDom.hide(this.modalBg);
		}
		else if(['refreshing', 'restoring'].indexOf(state) > -1)
		{
			_TinyDom.hide(this.iconDiv);
			_TinyDom.show(this.spinnerDiv);
			//this.acomp.enable(false);
			_TinyDom.show(this.modalBg);
		}
	}
	else {
		_TinyDom.hide(this.controlDiv);
		_TinyDom.hide(this.modalBg);
	}
};
...
{
	var thisObj = this;
    var curVal = _TinyDom.css(this.indicatorEle, 'transform');
	...
	Object.assign(this.scrollEle.style, {
		transform: 'translateY(0px)'
	});
};
...
RefreshIndicator.prototype.setIconColor = function(color)
{
	...
	this.iconDiv.setAttribute('fill', color);
};

RefreshIndicator.prototype.setSpinnerColor = function(color)
{
	this.spinnerColor = color;
	this.spinnerDiv.children[0].setAttribute('stroke', color);
};
...
RefreshIndicator.prototype.destroy = function()
{
	...
	this.indicatorEle.remove();
	this.indicatorEle = null;
