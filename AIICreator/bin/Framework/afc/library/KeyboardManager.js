...
KeyboardManager.getFocus = function(doc)
{
	...
	if(focusEl && focusEl.tagName == 'IFRAME')
	{
		focusEl = KeyboardManager.getFocus(focusEl.contentDocument);
	}
...
KeyboardManager.onKeyBoardShow = function(displayHeight, keyboardHeight, focusComp)
{
	...
	if(focusComp) focusEle = focusComp.getElement();
		...
		var focusEl = KeyboardManager.getFocus(document);
		if(!focusEl) focusEl = document.querySelector(':focus');
	...
	{
		var minMargin = 0,
			fTopOffset = _TinyDom.offset(focusComp.element).top;
		...
		else if(!KeyboardManager.oriPos)
		{
			...
			if(fTopOffset < minMargin)
			{
				topWnd.element.style.top = (topWnd.getPos().top + (-1 * fTopOffset) + minMargin) + 'px';
			}
		}
	}
	else
	...
	function _add_scrlMaker(_cntr)
	{
		...
		else
		{
			...
			if(!_scrlMaker)
			{
				_scrlMaker = document.createElement('div');
				_scrlMaker.id = 'scroll-maker';
				_cntr.element.appendChild(_scrlMaker);

				KeyboardManager.container = _cntr;
				KeyboardManager.prevOverflow = _TinyDom.css(_cntr.element, 'overflow');
				_cntr.element.style.overflow = 'auto';
			}

			Object.assign(_scrlMaker.style, {position:'absolute', width:'100%', height:keyboardHeight+'px', bottom:(-1*keyboardHeight)+'px'});
		}
...
KeyboardManager.onKeyBoardHide = function()
{
	...
	if(topWnd)
	{
		if(topWnd.element)
		{
			...
			Object.assign(topWnd.element.style, KeyboardManager.oriPosInfo);
	...
	else
	{
		...
		if(_cntr && _cntr.element)
		{
			...
			if(KeyboardManager.prevOverflow) _cntr.element.style.overflow = KeyboardManager.prevOverflow;
		}
		...
		if(!KeyboardManager.resizeWebview) {
			...
			var scrlMaker = document.getElementById('scroll-maker');
			if(scrlMaker) scrlMaker.remove();
