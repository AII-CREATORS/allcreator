...
//#######################################################################################
//remove jQuery $ele.html(cellData) --> ele.innerHTML = cellData  로 수정했는데.
//cellData 가 문자열인지 확인 필요.
//doRealPattern, doAddPattern, doUpdatePattern
...
class AGrid extends AComponent
{
    ...
    {
        const selRows = this.tBody.children
        ...
        if(state=='select') 
        {
            if(styleNames)
            {
                ...
                {
                    //row.classList.add(...styleNames.trim().split(' '))
                    _TinyDom.addClass(row, styleNames)
                    row.setAttribute('data-cur-applys', styleNames)
                }
            ...
            //스타일이 셋팅된 것이 없으면 아래 코드로 스타일 제거
        }

        for(const row of selRows)
        {
            const curStyles = row.getAttribute('data-cur-applys')
            if(curStyles) 
            {
                //row.classList.remove(...curStyles.trim().split(' '))
                _TinyDom.removeClass(row, curStyles)
                row.removeAttribute('data-cur-applys')
            }
        }
...
AGrid.CONTEXT = 
{
    tag: '<div data-base="AGrid" data-class="AGrid" data-fullrow-select="true" data-selectable="true" data-clear-rowtmpl="true" data-ver="3302" class="AGrid-Style" data-hide-footer="true">\
...
AGrid.prototype.initVariables = function()
{
	...
	this.headerTable = this.element.querySelector('.grid-header-table');
	this.bodyTable = this.element.querySelector('.grid-body-table');
	this.footerTable = this.element.querySelector('.grid-footer-table');
	this.scrollArea = this.bodyTable.parentElement;
	...
	*/
	if(!this.footerTable)
	{
		...
		div.innerHTML = '<table class="grid-footer-table" align="center"><tfoot align="center" class="foot-prop"></tfoot></table>'
		this.footerTable = div.firstElementChild

		this.scrollArea.append(this.footerTable);
		this.footerTable.prepend(this.headerTable.querySelector('colgroup').cloneNode(true));
		this.footerTable.querySelector('tfoot').append(...this.headerTable.querySelector('thead').cloneNode(true).children);
		this.element.setAttribute('data-hide-footer', true);
	}	

	//버전에 따라 thead가 없는 경우 헤더테이블의 thead 복사
	/*
	...
	*/
	if(!this.bodyTable.querySelector('thead'))
	{
		const clone = this.headerTable.querySelector('thead').cloneNode(true);
		this.bodyTable.querySelector('tbody').before(clone);
	}
	...
	*/
	if(!this.footerTable.querySelector('thead'))
	{
		const clone = this.headerTable.querySelector('thead').cloneNode(true);
		clone.querySelector('tr').style.height = '0px';
		clone.querySelector('td').textContent = '';
		this.footerTable.querySelector('tfoot').before(clone);
	}
	...
	if(!this.isDev())
	{
		...
		this.scrollArea.style.zIndex = 0;		//스크롤 가속을 위해 필요.
	}

	this.showThead = this.headerTable.querySelector('thead');
	const hideThead = this.bodyTable.querySelector('thead');
	const hideFhead = this.footerTable.querySelector('thead');
   	hideThead?.remove();
	hideFhead?.remove();

	//tbody 태그는 자주 쓰이므로 변수로 저장해 둔다.
	this.tBody = this.bodyTable.querySelector('tbody');
	this.tFoot = this.footerTable.querySelector('tfoot');
	...
	this.rowTmpl = this.tBody.cloneNode(true).children;
	//풋
	...
	this.columnCount = this.hRowTmpl[0].children.length;
	...
	//this.$tmplTds = [];
	//바디 로우 템플릿의 높이를 구해둔다.
	for(let i=0; i<this.$rowTmpl.length; i++)
	{
	...
	*/
	...
	let row, cells, rowHeight, cellHeight;
	//this.tmplTds = [];
	//바디 로우 템플릿의 높이를 구해둔다.
	for(let i=0; i<this.rowTmpl.length; i++)
	{
		...
		//Number() 로 파싱하면 22px 가 0 으로 나옴. parseInt 사용하기
		rowHeight = parseInt(row.getAttribute('height'));
		cellHeight = parseInt(cells[0].getAttribute('height'));
...
AGrid.prototype.init = function(context, evtListener)
{
	...
	//개발영역을 판단하는 기준을 afc 로 하였으나 Attribute에서는 동작을 해야하므로 수정
	if(!this.isDev())
	{
		//this.selectStyleName = this.$ele.attr('data-select-class');
		this.selectStyleName = this.element.getAttribute('data-select-class');
	...
	else
	{
		...
		*/
		for(const tr of this.showThead.children) 
		{
	...
	else
	{
		...
		*/
		for(const tr of this.tFoot.children) 
		{
			for(const td of tr.children) 
				td.isFooter = true;
		}
...
AGrid.prototype.layComponent = function(acomp, row, col, width, height)
{
	...
	*/
	...
	this.getCell(row, col)?.appendChild(acomp.element);
};
...
*/
	...
	this.getHeaderCell(row, col)?.appendChild(acomp.element);
};
...
AGrid.prototype.getColumnComps = function(colInx)
{
	...
	*/
	this.getRows().forEach(function(tr)
	{
		cell = tr.children[colInx];
...
AGrid.prototype.getDataMask = function(rowIdx, colIdx, idx)
{
    ...
    if(typeof(rowIdx)=="number")
	{
		...
		row = this.rowTmpl[rowIdx]; //tbody tr
	}
    else row = rowIdx;
	
    return AComponent.prototype.getDataMask(idx, row.children[colIdx]);
};
...
AGrid.prototype.loadGridDataMask = function()
{
	...
	if(!this.option.isHideFooter)
	{
		...
		for(i=0; i<tFootChildren.length; i++)
		{
			cells = tFootChildren[i].children;
			for(j=0; j<cells.length; j++)
			{
				this.loadDataMask(cells[j]);
			}
		}
	}
};
...
AGrid.prototype.loadGridNameKeyInfo = function(enable)
{
	...
	*/
	...
	if(this.rowTmpl)
	{
		let nameArr = this.nameKeyArr = [], name, idx = 0;
		
		for(const tr of this.rowTmpl)
		{
			for(const td of tr.children)
			{
...
AGrid.prototype.setFlexibleRow = function(enable)
{
	...
	if(!this.scrollArea.parentElement) tableParent = this.element;
	...
	if(enable)
	{
        ...
        */
        ...
        [...this.rowTmpl].forEach(ele => ele.style.height = 'auto' )
		this.tBody.querySelectorAll('tr').forEach(ele => ele.style.height = 'auto')
		this.scrollArea.style['overflow-y'] = 'hidden';
		
		this.bodyTable.style['flex-grow'] = 1;
		tableParent.style.display = 'flex'
		tableParent.style['flex-direction'] = 'column'
	}
	else
	{
        [...this.rowTmpl].forEach(ele => ele.style.height = '' )
        this.tBody.querySelectorAll('tr').forEach(ele => ele.style.height = '')
		this.scrollArea.style['overflow-y'] = '';
		
		this.bodyTable.style['flex-grow'] = '';
		tableParent.style.display = ''
		tableParent.style['flex-direction'] = ''
	}
...
AGrid.prototype.setScrollAreaId = function(areaId)
{
	...
	this.scrollArea.setAttribute('id', areaId);
};
...
AGrid.prototype.enableScrlManager = function(leftSyncArea, rightSyncArea)
{
	...
	this.scrollArea.style['-webkit-overflow-scrolling'] = '';	//ios overflow-scrolling delete
...
AGrid.prototype.setScrollArrow = function(headHeight)
{
	...
	this.sa.arrow1.style.top = headHeight+'px';
};
...
AGrid.prototype.scrollImplement = function(leftSyncArea, rightSyncArea) 
{
	let thisObj = this, 
		...
		//PC인 경우 자신의 영역 mousedown 과 상관없이 mousemove 가 무조건 발생한다.
	 	isDown = false,
		scrlArea = this.scrollArea,
		transTop, scrlFunc = _scrlHelper, initFunc = _initHelper;
...
AGrid.prototype.getRowData = function(row)
{
	...
	//jQuery객체 → afc 2.0 이벤트 info(배열) 언래핑 (0.5 $(row) 정규화 복원)
	else if(Array.isArray(row)) row = row[0];
...
AGrid.prototype.setRows = function(infoArr2, rowData2)
{
	...
	*/
	...
	let rowArr = [], rowSet, i;
	for(i=0; i<infoArr2.length; i++)
	{
		//rowSet 은 여러개의 tr, Fragment
		rowSet = this.createRow(infoArr2[i]);
		
		if(rowData2) rowSet.children[0]._data = rowData2[i];
    ...
    //기존 항목을 지우고 새로운 Elements 추가
	this.tBody.replaceChildren(...rowArr);
...
AGrid.prototype.addRow = function(infoArr, rowData)
{
    ...
    */
    ...
    //rowSet 은 여러개의 tr, Fragment
	let rowSet = this.createRow(infoArr);
	//Fragment 는 append 시 비워지므로 tr 들을 미리 배열로 확보 (0.5 의 rowset 컬렉션 반환 의미 복원)
	let rows = Array.from(rowSet.children);

	//row 전체를 대표하는 메모리 데이터
	if(rowData && rows[0]) rows[0]._data = rowData;

	if(this.bkManager && this.bkManager.appendItemManage(rowSet) )
	{
		return rows;
	}

	this.tBody.append(rowSet);
...
AGrid.prototype.prependRow = function(infoArr, rowData)
{
    //rowSet 은 여러개의 tr, Fragment
	let rowSet = this.createRow(infoArr);
	//Fragment 는 prepend 시 비워지므로 tr 들을 미리 배열로 확보 (0.5 의 rowset 컬렉션 반환 의미 복원)
	let rows = Array.from(rowSet.children);
	...
	if(this.bkManager && this.bkManager.prependItemManage(rowSet) ) return rows;

	this.tBody.prepend(rowSet);
...
AGrid.prototype.insertRow = function(nRow, infoArr, rowData)
{
    ...
    */
    ...
    //rowSet 은 여러개의 tr, Fragment
	let rowSet = this.createRow(infoArr);
	//Fragment 는 삽입 시 비워지므로 tr 들을 미리 배열로 확보 (0.5 의 rowset 컬렉션 반환 의미 복원)
	let rows = Array.from(rowSet.children);
...
AGrid.prototype.removeRow = function(rowIdx)
{
	...
	//rowSet(여러 tr) 배열도 처리 — addRow/이벤트 info 반환형과 통일
	else if(Array.isArray(rowIdx)) rowIdx.forEach(function(tr){ tr.remove(); });
	else rowIdx.remove();
...
AGrid.prototype.removeHeaderRow = function(rowIdx)
{
	if(typeof(rowIdx)=="number") 
	{
		this.showThead.children[rowIdx].remove();
	}
	else rowIdx.remove();
};
...
AGrid.prototype.removeFooterRow = function(rowIdx)
{
	if(typeof(rowIdx)=="number") 
	{
		this.tFoot.children[rowIdx].remove();
	}
	else rowIdx.remove();
};
...
AGrid.prototype.getDataByOption = function(rowInfo)
{
    ...
    */
	...
	{
        //_data 로 변경되었으므로 추가
        if(rowInfo[0]._data) return rowInfo[0]._data;
		else if(rowInfo[0].oridata) return rowInfo[0].oridata;
		else
		{
			let retData = [], tdArr = null, i, j;
			for(i=0; i<rowInfo.length; i++)
			{
				tdArr = rowInfo[i].children;
				for(j=0; j<tdArr.length; j++)
					retData.push(tdArr[j].textContent);	
			}
			return retData;
		}
	} 
...
AGrid.prototype.getRowSet = function(rowIdx)
{
    ...
    */
	let rowSetLength = this.rowTmpl.length,
		startIdx = rowSetLength*rowIdx;
...
AGrid.prototype.getRowIndexByInfo = function(rowInfo)
{
    ...
    return this.indexOfRow(rowInfo[0]);
};
...
AGrid.prototype._addCell = function(cellArr)
{
	...
	{
		//td 에 직접 추가해야 하므로
        for(const tr of cellArr)
        {
            for(const td of tr.children)
                //td.classList.add(this.selectStyleName);
                _TinyDom.addClass(td, this.selectStyleName);
        }
	}
	
	else 
    {
        for(const td of cellArr)
            //td.classList.add(this.selectStyleName);
            _TinyDom.addClass(td, this.selectStyleName);
    }
};
...
//----------------------------------------------------------------
//   select cell  ************* 민수차장 검토 필요 ****************
//----------------------------------------------------------------

//	isFullRowSelect 가 true 이면 cellArr 은 tr element 의 array
//	false 이면 td element 의 array

//	cellArr 는 element 를 담고 있는 배열
//	그룹지어야 할 td 나 tr 들을 모아서 배열로 넘긴다.

//	※ 주의, cellArr 는 특정 td 나 tr 들을 그룹짓고 있는 배열이므로 
//	동등 비교를 할 경우 this.selectedCells[i][0] === cellArr[0] 과 같이 해야 함.
//	그룹지어져 있는 경우 첫번째 원소의 주소만 비교하면 같은 그룹임
...
AGrid.prototype.selectCell = function(cellArr, e)
{
    ...
    //이전 버전과의 호환을 위해
    if(window.jQuery && cellArr instanceof jQuery) cellArr = cellArr.get()
	
	if(!e) e = {};
	
	//멀티셀렉트인 경우 모바일은 컨트롤키가 눌린것과 같이 동작한다.
	let isCtrlKey = e.ctrlKey||afc.isMobile, isShiftKey = e.shiftKey;
    const thisObj = this;
	...
	else if(isShiftKey)
	{
		if(this.option.isFullRowSelect)
		{
			...
			let firstIdx = this.indexOfRow(this.lastSelectedCell[0]),
			    lastIdx = this.indexOfRow(cellArr[0]),
			    i = firstIdx - lastIdx;
		...
		{
			const _indexOfCell = function(cell)
			{
				//const row = cell.parentElement;
				//return [ Array.from(row.parentElement.children).indexOf(row), Array.from(row.children).indexOf(cell)];

                return [ cell.parentElement.rowIndex - thisObj.hRowTmpl.length, cell.cellIndex ];
			}
			
			this.shiftCurrentCell = cellArr;
			
			let startRowIdx, endRowIdx, startColIdx, endColIdx, 
				hCellArr = [], bCellArr = [], fCellArr = [], func, cell;
			
			// 기존에 shift로 선택한 셀들이 있으면 선택해제 처리한다.
			for(let i=0; i<this.shiftSelectedCells.length; i++)
			{
				cell = this.shiftSelectedCells[i];
				if(cell) this.deselectCell( [cell] );
			}
            ...
            let selEle = this.lastSelectedCell[0]

            //push lastSelectedCell
			if(this.isHeadCell(selEle)) hCellArr.push(_indexOfCell(selEle));
			else if(this.isFootCell(selEle)) fCellArr.push(_indexOfCell(selEle));
			else bCellArr.push(_indexOfCell(selEle));
			...
			if(hCellArr.length == 1 || bCellArr.length == 1 || fCellArr.length == 1)
			{
				const setCell = function(startRowIdx, endRowIdx, func)
				{
...
// isFullRowSelect 가 참이면 cellArr 은 tr array 거짓이면 td array
AGrid.prototype.deselectCell = function(cellArr)
{
	if(this.option.isFullRowSelect)
	{
        ...
        {
            for(const td of tr.children)
                //td.classList.remove(this.selectStyleName);
                _TinyDom.removeClass(td, this.selectStyleName);
        }

	}
	else 
    {
        //$(cellArr).removeClass(this.selectStyleName);
        for(const td of cellArr)
            //td.classList.remove(this.selectStyleName);        
            _TinyDom.removeClass(td, this.selectStyleName);
    }
...
AGrid.prototype.clearAll = function()
{
    ...
    */
    this.tBody.querySelectorAll('td').forEach((ele)=>
    {
        ele.textContent = '';
		if(ele.dm) ele.dm.setOriginal('');
...
AGrid.prototype.clearSelected = function()
{
    ...
    */
	...
	if(this.option.isFullRowSelect)
	{
		for(const arr of this.selectedCells) 
		{
            for(const tr of arr)
            {
			    for(const td of tr.children)
	...
	{
        //for(const td of this.selectedCells) 
        //    td.classList.remove(this.selectStyleName);
        for(const arr of this.selectedCells) 
		{
            for(const td of arr)
...
{
    //basicStyle.css 의 .grid-body-table 참조
    //body table 의 첫번째 로우의 td 들은 header가 있는 경우 top 보더를 없앤다. 
    this.bodyTable.style.setProperty('--body-top-border', '0px');

	//this.showThead.show();
    _TinyDom.show(this.showThead);
	this.option.isHideHeader = false;
};

AGrid.prototype.hideHeader = function()
{
    this.bodyTable.style.setProperty('--body-top-border', '1px');

    //this.showThead.hide();
    _TinyDom.hide(this.showThead)
...
{
    //basicStyle.css 의 .grid-body-table 참조
    //body table 의 마지막 로우의 td 들은 footer가 있는 경우 bottom 보더를 없앤다.
    this.bodyTable.style.setProperty('--body-bottom-border', '0px');

	//this.tFoot.show();
    _TinyDom.show(this.tFoot)
...
{
    this.bodyTable.style.setProperty('--body-bottom-border', '1px');

    //this.tFoot.hide();
    _TinyDom.hide(this.tFoot)
...
AGrid.prototype.findRowByCellText = function(nCol, text)
{
    ...
    for(const tr of this.tBody.children)
	{
		if(this.getCellText(tr, nCol)==text)
		{
			retRow = tr;
			break;
		}
...
AGrid.prototype.findRowByCellData = function(nCol, data)
{
    ...
    for(const tr of this.tBody.children)
	{
		if(this.getCellData(tr, nCol)==data)
		{
			retRow = tr;
			break;
		}
...
AGrid.prototype.getFooterRowCount = function()
{
    ...
    return this.tFoot.children.length;
};
...
AGrid.prototype.getCell = function(rowIdx, colIdx)
{
    ...
    */
    let row = null;
    if(typeof(rowIdx)=="number") row = this.tBody.children[rowIdx]; //tbody tr
    else if(Array.isArray(rowIdx)) row = rowIdx[0];   // 이벤트 info(배열) → 0.5 $(rowIdx) 정규화 복원
    else row = rowIdx;

	return row?.children[colIdx];
};
...
{
    let row = null;
    if(typeof(rowIdx)=="number") row = this.showThead.children[rowIdx]; //tbody tr
    else row = rowIdx;
...
{
    let row = null;
    if(typeof(rowIdx)=="number") row = this.tFoot.children[rowIdx]; //tbody tr
    else row = rowIdx;
...
AGrid.prototype.getRow = function(rowIdx)
{
	...
	return this.tBody.children[rowIdx];
};
...
AGrid.prototype.getFirstRow = function()
{
    ...
    return this.tBody.firstElementChild;
};
...
AGrid.prototype.getRows = function(start, end)
{
	...
	if(start!=undefined) return Array.from(this.tBody.children).slice(start, end);
	else return Array.from(this.tBody.children);
...
AGrid.prototype.getCellTag = function(rowIdx, colIdx)
{
    ...
    return this.getCell(rowIdx, colIdx).innerHTML;
};
...
AGrid.prototype.indexOfCell = function(cell)
{
	...
	//const row = cell.parentElement; 
    //return [this.indexOfRow(row), Array.from(row.children).indexOf(cell)];
    //-->
    //이벤트 info / getSelectedCells()[0] (배열) 언래핑 — getCell 등과 동일 컨벤션
    if(Array.isArray(cell)) cell = cell[0];
    return [ this.indexOfRow(cell.parentElement), cell.cellIndex];
...
AGrid.prototype.indexOfRow = function(row)
{
	...
	if(!row) return -1;
	//afc 2.0 이벤트 info(배열) 언래핑 (0.5 $(row) 정규화 복원)
	if(Array.isArray(row)) row = row[0];
	if(!row) return -1;

    // afc 2.0.0 에서 row.rowIndex - hRowTmpl.length 로 계산하던 것을
    // header/body/footer 가 각각 별개 <table> 이므로 body row 의 rowIndex 는
    // 이미 0 부터 시작 → hRowTmpl.length 를 빼면 안 됨 (클릭/선택 위치 어긋남)
    //let inx = row.rowIndex - this.hRowTmpl.length
    //let inx = Array.from(this.tBody.children).indexOf(row);     // O(n)
    let inx = row.rowIndex;		                                  // O(1)
	if(inx < 0) return -1;
...
{
	if(Array.isArray(cell)) cell = cell[0];	//getSelectedCells()[0]/이벤트 info(배열) 언래핑
	return this.indexOfRow(cell.parentElement);
};
...
AGrid.prototype.setHeaderCellText = function(rowIdx, colIdx, txt)
{
    ...
    this.getHeaderCell(rowIdx, colIdx).textContent = txt;
};
...
AGrid.prototype.loadCellView = function(rowIdx, colIdx, url)
{
    ...
    */
    ...
    const cell = this.getCell(rowIdx, colIdx);	//td
    const item = document.createElement('div');

    Object.assign(item.style, { width: '100%', height: '100%', overflow: 'auto' });
	
    cell.replaceChildren(item);
    
	return AView.createView(item, url, this);
};
...
//-------------------------------------------------------------------------------------
//  SpiderGen_Client DataPropertiesDlg 에서 호출됨

AGrid.prototype.setCellTextColor2 = function(cell, color)
{	
	let cellIndex = this.indexOfCell(cell), headCell;
	
	if(this.isHeadCell(cell))
		headCell = this.showThead.children[cellIndex[0]].children[cellIndex[1]]
...
AGrid.prototype.getCellTextColor2 = function(cell) { return cell.style['color']; };
...
AGrid.prototype.getCellBgColor2 = function(cell) { return cell.style['background-color']; };

//deprecated, use this.getCellPos
...
AGrid.prototype.setCellHAlign = function (cell, align)
{
	if(this.isHeadCell(cell))
	{	
		const cellIndex = this.indexOfCell(cell);
		
		//var $headCell = $(this.showThead.children().eq(cellIndex[0]).children().get(cellIndex[1]));
		//$headCell.css('text-align', align);	

		const headCell = this.showThead.children[cellIndex[0]].children[cellIndex[1]];
		headCell.style['text-align'] = align;	
	}

	cell.style['text-align'] = align;//cell.css('text-align', align);
};
...
AGrid.prototype.setCellVAlign = function (cell,align)
{
	if(this.isHeadCell(cell))
	{
        ...
        const headCell = this.showThead.children[cellIndex[0]].children[cellIndex[1]];
		headCell.style['vertical-align'] = align;        
	}

	cell.style['vertical-align'] = align;//cell.css('vertical-align', align);
};
...
AGrid.prototype.setHeaderCellStyle = function(rowIdx, colIdx, key, value)
{
    ...
    this.getHeaderCell(rowIdx, colIdx).style[key] = value;
};

AGrid.prototype.setHeaderCellStyleObj = function(rowIdx, colIdx, obj)
{
	//$(this.getHeaderCell(rowIdx, colIdx)).css(obj);
    Object.assign(this.getHeaderCell(rowIdx, colIdx).style, obj)
};
...
AGrid.prototype.setCellStyleObj = function(rowIdx, colIdx, obj)
{
    ...
    Object.assign(this.getCell(rowIdx, colIdx).style, obj)
};
...
AGrid.prototype.getCellStyle = function(rowIdx, colIdx, key)
{
    ...
    return _TinyDom.css(this.getCell(rowIdx, colIdx), key);
...
AGrid.prototype.cellAddClass = function(rowIdx, colIdx, className)
{
    ...
    //this.getCell(rowIdx, colIdx).classList.add(className);
    _TinyDom.addClass(this.getCell(rowIdx, colIdx), className);
};

AGrid.prototype.cellRemoveClass = function(rowIdx, colIdx, className)
{
    //$(this.getCell(rowIdx, colIdx)).removeClass(className);
    //this.getCell(rowIdx, colIdx).classList.remove(className);
    _TinyDom.removeClass(this.getCell(rowIdx, colIdx), className);
};
...
AGrid.prototype.setCellHeight = function(cell, height)
{
	...
	var index = this.indexOfCell(cell);
	
	if(isHead)
	{
		//this.showThead.children('tr').eq(index[0]).children().eq(index[1]).attr('height', height);
		this.showThead.children[index[0]].children[index[1]].setAttribute('height', height);
	}
	else
    {
		//this.tBody.children('tr').eq(index[0]).children().eq(index[1]).attr('height', height);	
        this.tBody.children[index[0]].children[index[1]].setAttribute('height', height);
    }
};
...
AGrid.prototype.getCellHeight = function(cell)
{
    ...
    return cell.getAttribute('height');
};

// 헤더의 특정 Row 높이 리턴
AGrid.prototype.getHeadHeight = function(rowInx)
{
	if(rowInx == undefined) rowInx = 0;
	//return this.showThead.children('tr').eq(rowInx).attr('height');
    return this.showThead.children[rowInx].getAttribute('height');
};
...
AGrid.prototype.setHeadHeight = function(headHeight)
{
	...
	for(const tr of this.showThead.children)
        tr.setAttribute('height', headHeight)
};

// 바디 특정 Row 의 높이 리턴.
AGrid.prototype.getBodyHeight = function(rowInx)
{
	if(rowInx == undefined) rowInx = 0;
	//return this.tBody.children('tr').eq(rowInx).attr('height');
    return this.tBody.children[rowInx].getAttribute('height');
};
...
AGrid.prototype.setBodyHeight = function(bodyHeight)
{
    ...
    for(const tr of this.tBody.children)
        tr.setAttribute('height', bodyHeight);
};

//풋터 특정 Row 의 높이 리턴
AGrid.prototype.getFootHeight = function(rowInx)
{
	if(rowInx == undefined) rowInx = 0;
	//return this.tFoot.children('tr').eq(rowInx).attr('height');
    return this.tFoot.children[rowInx].getAttribute('height');
};
...
AGrid.prototype.setFootHeight = function(footHeight)
{
    ...
    for(const tr of this.tFoot.children)
        tr.setAttribute('height', footHeight);
};
...
AGrid.prototype.getHeadColor = function()
{
    ...
    return _TinyDom.css(this.showThead.children[0], 'background-color');
};

AGrid.prototype.getFootColor = function()
{
	//return this.tFoot.children('tr').eq(0).css('background-color');
    return _TinyDom.css(this.tFoot.children[0], 'background-color');
};
...
AGrid.prototype.addColumn = function(width)
{
    ...
    */
    ...
    this.headerTable.querySelector('colgroup').append(`<col width="${width}"></col>`);
	this.footerTable.querySelector('colgroup').append(`<col width="${width}"></col>`);
    this.bodyTable.querySelector('colgroup').append(`<col width="${width}"></col>`);
	
    _add_helper(this.showThead.children);
    _add_helper(this.tBody.children);
	_add_helper(this.tFoot.children);
	_add_helper(this.rowTmpl);

	this.columnCount++;
		
	function _add_helper(elements)
	{
...
AGrid.prototype.removeColumn = function(colIdx)
{
    ...
    */
	...
	this.headerTable.querySelector('colgroup').children[pos].remove();
	this.footerTable.querySelector('colgroup').children[pos].remove();
	this.bodyTable.querySelector('colgroup').children[pos].remove();
	
	_remove_helper(this.showThead.children);
    _remove_helper(this.tBody.children);
	_remove_helper(this.tFoot.children);
	_remove_helper(this.rowTmpl);
        
    this.columnCount --;
	
	function _remove_helper(elements)
	{
        let removeTd
        for(const ele of elements)
        {
            removeTd = ele.children[pos];
            thisObj.decreaseColSpan(removeTd);
            removeTd.remove();
        }
	}
...
AGrid.prototype.hideColumn = function(colIdx)
{
	...
	let colCnt = this.getColumnCount(),
    ...
    */
	this.headerTable.querySelectorAll('col').forEach((ele, i)=>
	{
...
/*
	var changeShowHTarget, changeHideHTarget, changeBTarget, changeFTarget, 
...
*/
	let changeShowHTarget, changeBTarget, changeFTarget, 
		showTheadArr = this.showThead.children, //tr, tr ... 
		bodyArr = this.tBody.children,
		footArr = this.tFoot.children;

	this.headerTable.querySelectorAll('col')[colIdx].style.width = '0px';
	this.footerTable.querySelectorAll('col')[colIdx].style.width = '0px';
	this.bodyTable.querySelectorAll('col')[colIdx].style.width = '0px';

    const selector = 'td:nth-child('+(colIdx+1)+')'

	changeShowHTarget = _TinyDom.group(showTheadArr, selector);
	changeBTarget = _TinyDom.group(bodyArr, selector);
	changeFTarget = _TinyDom.group(footArr, selector);

	this.oriWhiteSpace[colIdx] = [
        _TinyDom.group(changeShowHTarget, ele => ele.style['white-space']),
        _TinyDom.group(changeBTarget, ele => ele.style['white-space']),
        _TinyDom.group(changeFTarget, ele => ele.style['white-space']),
	];

    /*
	changeShowHTarget.style['white-space'] = 'nowrap';
	changeHideHTarget.style['white-space'] = 'nowrap';
	changeBTarget.style['white-space'] = 'nowrap';
	changeFTarget.style['white-space'] = 'nowrap';
    */
	_TinyDom.group(changeShowHTarget, ele => ele.style['white-space'] = 'nowrap')
	_TinyDom.group(changeBTarget, ele => ele.style['white-space'] = 'nowrap')
	_TinyDom.group(changeFTarget, ele => ele.style['white-space'] = 'nowrap')
	
	if(this.option.isColumnResize && this.resizeBars[colIdx])
	{
		this.resizeBars[colIdx].style.display = 'none';
...
AGrid.prototype.showColumn = function(colIdx)
{
	...
	let showTheadArr = this.showThead.children, //tr, tr ... 
		bodyArr = this.tBody.children,
		footArr = this.tFoot.children;
	
	this.headerTable.querySelectorAll('col')[colIdx].style.width = '';
	this.footerTable.querySelectorAll('col')[colIdx].style.width = '';
	this.bodyTable.querySelectorAll('col')[colIdx].style.width = '';

    const selector = 'td:nth-child('+(colIdx+1)+')'

	let changeShowHTarget = _TinyDom.group(showTheadArr, selector),
		changeBTarget = _TinyDom.group(bodyArr, selector),
		changeFTarget = _TinyDom.group(footArr, selector);

    const oriWS = this.oriWhiteSpace[colIdx]
	if(oriWS)
	{
        _TinyDom.group(changeShowHTarget, ele => ele.style['white-space'] =  oriWS[0]||'')
        _TinyDom.group(changeBTarget, ele => ele.style['white-space'] = oriWS[1]||'')
        _TinyDom.group(changeFTarget, ele => ele.style['white-space'] = oriWS[2]||'')
	...
	if(this.option.isColumnResize && this.resizeBars[colIdx])
	{
        this.resizeBars[colIdx].style.display = '';
...
AGrid.prototype.getColumnWidth = function(colIdx)
{
    ...
    return this.headerTable.querySelectorAll('col')[colIdx].getAttribute('width');
};
...
AGrid.prototype.setColumnWidth = function(colIdx, width, noUpdateBar)
{
	...
	this.headerTable.querySelectorAll('col')[colIdx].setAttribute('width', width);
	this.footerTable.querySelectorAll('col')[colIdx].setAttribute('width', width);
	this.bodyTable.querySelectorAll('col')[colIdx].setAttribute('width', width);
...
AGrid.prototype.getAllColumnName = function()
{
    ...
    */
    let retArr = []
    let tmplCells = this.hRowTmpl[0].children

    for(const ele of tmplCells)
        retArr.push(ele.innerText)
...
AGrid.prototype.getAllColumnInfo = function()
{
    ...
    for(let inx=0; inx<this.hRowTmpl.length; inx++)
    {
        tmpArr = [], colArr = [], before = '', colSpan = 0

        //$(ele1).children('td').each((i, ele) =>
        for(const ele of this.hRowTmpl[inx].children)
        {
            if(inx==lastInx) 
            {
                tmpArr.push(ele.innerText)
                colArr.push(0)
            }
            else 
            {
                tmp = ele.getAttribute('colspan')
                if(tmp) 
                {
                    colSpan = Number(tmp)
                    colArr.push(colSpan)
                ...
                if(ele.innerText) before = ele.innerText
                tmpArr.push(before)
...
AGrid.prototype.isHiddenColumn = function(colIdx)
{
	...
	if(this.headerTable.querySelectorAll('col')[colIdx].style.width == '0px') return true;
	else return;
};
...
{
	let arr = this.rotateColArr[index];
...
AGrid.prototype.decreaseColSpan = function(tdDom)
{
	...
	if(curColSpan)
	{
		let newColSpan = parseInt(curColSpan, 10) - 1;
...
// 	this.makeDefaultCellStyle($newCell);
	...
	let newCell = document.createElement('td'),
	    fromCell = table.children[row].children[col];

	if(isAfter) fromCell.after(newCell);
	else fromCell.before(newCell);
};
...
AGrid.prototype.insertSingleCol = function(colIndex, isAfter)
{
	...
	{
		//let fromCol = table.find('colgroup').children('col').eq(i),
		//    newCol = $('<col></col>');

		let fromCol = table.querySelector('colgroup').children[i],
...
// 		this.makeDefaultCellStyle($cell);
    ...
    */
	...
	if(areaType == 'thead')
		rowHeight = this.showThead.children[0].getAttribute('height');
	else if(areaType == 'tfoot')
		rowHeight = this.tFoot.children[0].getAttribute('height');
	else
		rowHeight = this.tBody.children[0].getAttribute('height');
	...
	for(let i=0; i<this.columnCount; i++)
	{
		...
		// 숨겨진 컬럼이 있는 경우 숨김 처리를 해준다.
		if(this.isHiddenColumn(i)) cell.style['white-space'] = 'nowrap';
// 		this.makeDefaultCellStyle(cell);
	...
	if(areaType == 'thead')
	{
        ...
        _TinyDom.height(clone, 0)
		if(isAfter)
		{		
			this.showThead.children[rowIndex].after(row);
		}
		else
		{
			this.showThead.children[rowIndex].before(row);
		}
	}
	else
	{
		...
		if(isAfter)
			area.children[rowIndex].after(row);
		else
			area.children[rowIndex].before(row);
	}
...
// 		this.makeDefaultCellStyle($cell);
	...
	let color = this.getFootColor();
	let colors = color.substring(5, color.length -1).replace(/ /gi,'').split(',');
	if(! ((colors[0] == 0) && (colors[1] == 0) && (colors[2] == 0) && (colors[3] == 0)))
		row.style['background-color'] = color;
	...
	if(isAfter)
	{		
		this.tFoot.children[rowIndex].after(row);
	}
	else
	{
		this.tFoot.children[rowIndex].before(row);
	}
...
AGrid.prototype.changeRowCount = function(count, isHead)
{
	let calcValue = 0;
	let thisObj = this;
	let headRowCnt = this.showThead.children.length;
	let bodyRowCnt = this.tBody.children.length;
	...
	function _removeRow_helper(isHead)
	{
        ...
        */
		if(isHead)
		{
			target = thisObj.showThead.children[pos];
			for(const ele of target.children) {
				_decreaseRowSpan(ele);	
			}
			target.remove();
		...
		{
			target = thisObj.tBody.children[pos]; 
			for(const ele of target.children) {
				_decreaseRowSpan(ele);	
				//$(thisObj).remove();	
			};
	...
	function _decreaseRowSpan(tdDom)
	{
        /*
		let tdObj = $(tdDom);
		let tdIdx = tdObj.index();

		if(tdObj.attr('rowspan'))
		{
			let curRowSpan = tdObj.attr('rowspan');
			let newRowSpan = parseInt(curRowSpan, 10) - 1;
		...
		else
		{
			...
			if(tdDom.style.display == 'none')
			{
				let preTrIndex = tdObj.parent().prev();
				let preTd = preTrIndex.children('td').eq(tdIdx);
				if(preTd[0]) _decreaseRowSpan(preTd[0]);
			}
			else return true;	
		}
        */
		let tdIdx = tdDom.cellIndex;

		if(tdDom.getAttribute('rowspan'))
		{
			let curRowSpan = tdDom.getAttribute('rowspan');
			let newRowSpan = parseInt(curRowSpan, 10) - 1;

			if(newRowSpan <= 1) tdDom.removeAttribute('rowspan');
			else tdDom.setAttribute('rowspan', newRowSpan);
			return false;
		}  
		else
		{
			if(tdDom.style.display == 'none')
			{
				let preTrIndex = tdDom.parentElement.previousElementSibling;
				let preTd = preTrIndex.children[tdIdx];
				if(preTd) _decreaseRowSpan(preTd);
			}
			else return true;	
		}
...
AGrid.prototype.changeFootRowCount = function(count)
{
	let calcValue = 0;
	let thisObj = this;
	let footRowCnt = this.tFoot.children.length;
	...
	function _removeRow_helper()
	{
		...
		let target = thisObj.tFoot.children[pos];
		for(const ele of target.children) {
			_decreaseRowSpan(ele);	
		}
		target.remove();
...
{
	this.scrollArea.scrollTop = (this.scrollArea.scrollHeight - this.element.offsetHeight)/2;
};

AGrid.prototype.saveScrollPos = function()
{
	this.savedScrollPos = this.scrollArea.scrollTop;
};
...
AGrid.prototype.restoreScrollPos = function()
{
	...
	{
		this.scrollArea.scrollTop = this.savedScrollPos;
		this.savedScrollPos = -1;
	}
};
...
{
	return (this.scrollArea.scrollTop == 0);
};

AGrid.prototype.isScrollBottom = function()
{
	var scrlEle = this.scrollArea;
...
AGrid.prototype._makeRowset = function(collection)
{
...
//rowSet HTMLElement Array 를 리턴한다.
AGrid.prototype.createRow = function(rowData)
{
	...
	*/
    ...
    //rowSet is DocumentFragment
	let rowSet = null, trs;
	
	//템플릿이 있으면 복제하여 사용
	if(this.rowTmpl)
	{
		let idx = 0, cell, colSet, cellData, thisObj = this, tmplCells; 
		
        //rowSet is DocumentFragment
		rowSet = this._makeRowset(this.rowTmpl);	//<tr></tr> <tr></tr> ...
        trs = rowSet.children
		
		for(let i=0; i<trs.length; i++)
		{
			colSet = trs[i].children;	//<td><td> ...
			
			//tmplCells = this.tmplTds[i];
			tmplCells = this.rowTmpl[i].children;
			
			for(let j=0; j<colSet.length; j++)
			{
				cell = colSet[j];
				...
				if(cell.style.display != 'none')
				{
					...
					//템플릿의 data mask 객체를 셋팅한다.
					cell.dm = tmplCells[j].dm;
					
					if(rowData.length>idx)
					{
						if(cell.dm) cellData = cell.dm.mask(cellData, cell);
						...
						else if(cellData != undefined) cell.innerHTML = cellData;
						//else $cell.html(cellData);
					...
					if(tmplCells[j].shrinkInfo)
					{
						cell.shrinkInfo = tmplCells[j].shrinkInfo;
						AUtil.autoShrink(cell, cell.shrinkInfo) 
					}
...
AGrid.prototype.getData = function(dataArr, keyArr)
{
	...
	let rowTmplCount = this.rowTmpl.length,
	...
	{
		keyArr = [];
		for(let i=0; i<this.rowTmpl.children.length; i++) keyArr[i] = i;
		newFunc = _new_array;
	}
...
AGrid.prototype.setQueryData = function(dataArr, keyArr, queryData)
{
	...
	if(this.option.isClearRowTmpl)
	{
		if(queryData.isReal) 
		{
			...
			let realType = queryData.getRealType(this);
...
{
	let data, row, keyVal, arr, rowArr, idx, cell, cellData, i, j;
...
{
	let data, row, keyVal, arr, i, j, rowArr;
...
AGrid.prototype.doUpdatePattern = function(dataArr, keyArr, queryData)
{
	...
	//let $rowSet = this.getRowSet(0), $colSet, $cell, cellData, thisObj = this,
	//	data, keyVal, idx = 0;
	let rowSet = this.getRowSet(0), colSet, cell, cellData, thisObj = this,
		data, keyVal, idx = 0;
	...
	//$rowSet.each(function(i)
    rowSet.forEach(function(row, i)
	{
		//$colSet = $(this).children('td');	//<td><td> ...
		//var $tmplCells = thisObj.$rowTmpl.eq(i).children('td');

        let tmplCells = thisObj.rowTmpl[i].children;    //<td>
        ...
        for(let j=0; j<colSet.length; j++)
		{
			...
			if(cell.style.display != 'none')
			{
				...
				if(keyVal)
				{
					...
					//if(cell.shrinkInfo) $cell.autoShrink(cell.shrinkInfo);
                    if(cell.shrinkInfo) AUtil.autoShrink(cell, cell.shrinkInfo);
				}
				idx++;
			}
	...
	if(!this.option.isHideFooter)
	{
		for(const row of this.tFoot.children) //().each(function(i){
        {
			//$colSet = $(this).children('td');

			for(cell of row.children) //.each(function(j){
            {
				//$cell = $(this);
...
AGrid.prototype.getCompStyleObj = function()
{
	...
	*/
	let obj = {}, ele = this.getElement();
	obj.main = AUtil.getDefinedStyle(ele);
	obj.thead = AUtil.getDefinedStyle(ele.querySelector('thead'));
	obj.td = AUtil.getDefinedStyle(ele.querySelector('td'));
	obj.select = AUtil.getDefinedStyle(ele.querySelector('.agrid_select'));
...
AGrid.prototype.setCompStyleObj = function(obj)
{
	...
	*/
	let p, ele = this.getElement();
	for(p in obj.main) this.setStyle(p, obj.main[p]);
	for(p in obj.thead) this.showThead.style[p] = obj.thead[p];
	for(p in obj.td) ele.querySelector('td').style[p] = obj.td[p];
	for(p in obj.select) ele.querySelector('.agrid_select').style[p] = obj.select[p];
};
...
AGrid.prototype.getRowParentTag = function(cell)
{
    ...
    */
	...
	let parentTagStr = '';
	let parentDom = cell.parentElement?.parentElement;
	if(parentDom) parentTagStr = parentDom.tagName.toLowerCase();
	return parentTagStr;
};
...
AGrid.prototype.getMappingCount = function()
{
	...
	for(let i=0; i<this.rowTmpl.length; i++)//.each(function(i)
	{
        let tds = this.rowTmpl[i].children
        for(let j=0; j<tds.length; j++) //$(this).children().each(function(j)
		{
			if(tds[j].style.display != 'none') arr.push((i+1)+'-'+(j+1));
		}
	}
	...
	if(!this.option.isHideFooter)
	{
        ...
        for(let i=0; i<trs.length; i++)
		{
			...
			{
				if(tds[j].style.display != 'none') arr.push('f' + (i+1)+'-'+(j+1));
			}
		}
	}
...
//ADataGrid 샘플 중에 merge 오류 부터 작업
AGrid.prototype._mergeRow = function (row, col, span, rowEles)
{
	let start = row + 1;
	...
	let rowEle = rowEles[row],
		startCol = rowEle.children[col], curCol, addSpan;
	
	for(let index=start; index<end; ++index)
	{
		curCol = rowEles[index].children[col];
		
		if(index == end-1)
		{
			addSpan = curCol.getAttribute('rowspan');
			if(addSpan)
				totalSpan += (Number(addSpan) -1);
		}
...
{
	this._mergeCol(this.showThead.children[row], col, span);
};
...
{
	this._mergeCol(this.tFoot.children[row], col, span);
};
...
AGrid.prototype._mergeCol = function(row, colInx, span)
{
	// colspan td 다음부터(+1) 
	let start = colInx + 1; 
	let end = start + span - 1;
	...
	let startCol = row.children[colInx], addSpan;

	for(let index=start; index<end; ++index)
	{
		let curCol = row.children[index];
		// 마지막 col 에 colspan 존재하면 이를 더해야 함.
		// 중간의 colspan 은 이미 param 에 합쳐진 수치이므로 걍 attr 만 지우면 됨.		
		if(index == end -1)
		{
			addSpan = curCol.getAttribute('colspan');
			if(addSpan)
				totalSpan += (Number(addSpan)-1);
		}
...
{
	this._splitCell(row, col, this.showThead.children[row].children[col]);	
};
...
{
	this._splitCell(row, col, this.tFoot.children[row].children[col]);	
};
...
AGrid.prototype._splitCell = function(row, col, td)
{	
	...
	//정수형으로 만들기 위해 
	let rStart = Number(row);
	let rEnd = rStart + Number(rowSpanCount);
		
	let cStart = Number(col);
	let cEnd = cStart + Number(colSpanCount);

	let parent = td.parentElement.parentElement;
	let tr, curTd;
	for(let rIndex=rStart; rIndex<rEnd; ++rIndex)
	{
		tr = parent.children[rIndex];
		for(let cIndex=cStart; cIndex<cEnd; ++cIndex)
		{
			curTd = tr.children[cIndex];
            _TinyDom.show(curTd);
		}
	}
...
AGrid.prototype.splitRow = function(rowInx, colInx)
{
	//var $row = $(this.getRow(row));
    let row = this.getRow(rowInx);
	
	row.children[colInx].removeAttribute('rowspan'); //add ukmani100
};

AGrid.prototype.splitCol = function(rowInx, colInx)
{
	let row = this.getRow(rowInx);
	
	row.children[colInx].removeAttribute('colspan'); //add ukmani100
};

AGrid.prototype.showGridMsg = function(isShow)
{
    /*
	if(isShow)
	{
...
AGrid.prototype._getDataStyleObj = function()
{
	...
	let keyArr = ['data-select-class', 'data-style-header', 'data-style-body', 'data-style-footer'], val;
	...
	for(let i=0; i<keyArr.length; i++)
	{
		...
		else if(i==3) val = this.$ele.find('tfoot').attr(keyArr[i]);
...
AGrid.prototype._setDataStyleObj = function(styleObj)
{
	for(let p in styleObj)
	{
		...
		else if(p=='data-style-footer') this._set_class_helper(this.$ele.find('tfoot'), null, styleObj, p);	//화면에 바로 적용
        ...
        */
		...
		else if(p=='data-style-header') this._set_class_helper(this.element, this.element.querySelector('thead'), styleObj, p);	//화면에 바로 적용
		else if(p=='data-style-body') this._set_class_helper(this.element, this.element.querySelector('tbody'), styleObj, p);	//화면에 바로 적용
		else if(p=='data-style-footer') this._set_class_helper(this.element, this.element.querySelector('tfoot'), styleObj, p);	//화면에 바로 적용
		else this.setAttr(p, styleObj[p]);	//data-select-class, attr 값만 셋팅
	}
};
...
AGrid.prototype.sortColumn = function(colInx, isAsc)
{
	...
	if(typeof(colInx) != "number")
	{
		if(this.isHeadCell(colInx))
		{
			...
			let row = colInx.parentElement;
			let rIdx = this.indexOfRow(row) 
			if(rIdx < 0) rIdx = 0;

			//colInx = this.getColumnCount()*rIdx + $row.children().index(colInx);
            colInx = this.getColumnCount()*rIdx + colInx.cellIndex;
		}
		else return;
	}
	...
	else
	{
		if(isAsc == undefined)
		{
			if(this.sortImg.getAttribute('src').indexOf('down') > -1) isAsc = true;
			else isAsc = false;
		}
	}
	...
	if(this.sortImg) this.sortImg.setAttribute('src', 'Framework/afc/image/' + sortText + '.png');
	else
	{
		...
		cell.append('<img src="Framework/afc/image/' + sortText + '.png" style="vertical-align: middle; margin-left: 5px; margin-right: -21px"></img>');
		this.sortImg = cell.lastElementChild;
	}
    ...
    //let trArr = [].concat(...tbody.children);
    let trArr = Array.from(tbody.children);
	if(!isSorted) 
    {
		...
		let _sortResult;
		for (i=0; i<rowCnt-rsLen; i+=rsLen) 
		{
	...
	if(isSorted) 
    {
		...
		for (i=rowCnt-rsLen-1; i>-1; i-=rsLen) 
        {
			for(j=0; j<rsLen; j++) 
                tbody.append(trArr[i+j]);
...
AGrid.prototype.overscrollBehavior = function(disableScrlManager)
{
	...
	let thisObj = this, oldScrollTop, 
		scrlArea = this.scrollArea, startY = 0, isTouchLeave = false, isRemove = true;
...
AGrid.prototype.columnResizable = function(callback)
{
    ...
    */
	...
	let thisObj = this, pos, idx, isLastColumn = false,
		cols = this.headerTable.querySelectorAll('col'),
		colLen = cols.length, arr = new Array(colLen);

    //저장하지 않고 동적으로 구해온다.
    let headTds = this.hRowTmpl[0].children, td,
        endIdx = headTds.length - 1,//endIdx = headTds.length/hRowCount - 1,
        scaleX = this.getBoundRect().width / this.element.offsetWidth,
        headLeft = _TinyDom.position(this.showThead).left
	
	for(let i=0; i<headTds.length; i++)// headTds.each(function(i)
	{
        td = headTds[i]

        //moveColumn 시 오류나서 주석처리하고 width: 0으로 생성
		//if(td.style.display == 'none') return;		//앞에서 colspan 세팅했으므로 리턴
		//if(td.getAttribute('colspan') > 1) return;	//colspan 처리시 리턴
		...
		//pos = ($(this).position().left - headLeft) / scaleX + Math.max(this.offsetWidth, 4) - 4;
        pos = (_TinyDom.position(td).left - headLeft) / scaleX + Math.max(td.offsetWidth, 4) - 4;
		...
		*/
		...
		Object.assign(sizeBar.style, 
		{
			...
			left: pos + 'px',
			top: '0px',
			width: td.style.display=='none'?0:'4px',//isLastColumn?'3px':'5px', 
			height: '100%',
			'z-index': 100,
			//border: '1px solid blue'
		});
		...
		if(cols[idx].style.width == '0px') sizeBar.style.display = 'none';
		...
		if(isLastColumn && !thisObj.option.isWidthChangable) sizeBar.style.display = 'none';
		
		//리사이즈바가 헤더 안에서만 보이도록
		//thisObj.showThead.append($sizeBar);
		//thisObj.showThead.appendChild(sizeBar);
        thisObj.headerTable.appendChild(sizeBar);
...
AGrid.prototype._resizeProcess = function(sizeBar)
{
    ...
    { 
        axis: 'x',
        containment: this.option.isWidthChangable? null : "parent", //containment: 'window',
        cursor: "e-resize",
        //helper: "clone",

        onStart: function( {draggable} )
        {
			thisObj.element.style.userSelect = 'none';

            //scale 된 경우에도 정상동작하도록 수정
            thisObj._dragScaleX = (thisObj.getBoundRect().width / thisObj.element.offsetWidth).toFixed(2);
            //사이즈바 라인이 전체적으로 보이도록
            //thisObj.$ele.append($sizeBar);
			thisObj.element.appendChild(sizeBar);
            
            //스케일되기 전의 left 값으로 저장
            //sizeBar.beforeLeft = ($sizeBar.position().left + thisObj.showThead.position().left) / thisObj._dragScaleX;
			sizeBar.beforeLeft = (_TinyDom.position(sizeBar).left + _TinyDom.position(thisObj.showThead).left) / thisObj._dragScaleX;
        },
        ...
        onStop: function( {draggable} ) 
        {
			...
			thisObj.element.style.userSelect = 'auto';
			...
			*/
            ...
            thisObj._colculResizeWidth(sizeBar._inx, rt.left - prt.left);
        ...
        onDrag: function( {draggable} ) 
        {
            ...
            //스케일되기 전의 left 값으로 변경
            //ui.position.left = rt.left / thisObj._dragScaleX;
            //ui.position.top = 0; //top 값이 변경되어 0으로 초기화처리

            //스케일되기 전의 left 값으로 변경
            //top 값이 변경되어 0으로 초기화처리
            draggable.setVisualPosition(rt.left/thisObj._dragScaleX, rt.top)
        }
    });
	...
	*/
    ...
    //마우스 다운상태였다가 마우스업하면 리사이즈 가능상태 표현되도록 수정
    sizeBar.addEventListener('mouseup', function(e)
    {
        Object.assign(sizeBar.style, 
        {
            cursor: 'e-resize',
...
AGrid.prototype._updateBarPos = function()
{
	...
	for(let i=0; i<headTds.length; i++) //.each(function(i)
	{
        td = headTds[i]

		if(td.style.display == 'none') return;		//앞에서 colspan 세팅했으므로 리턴
		//if(td.getAttribute('colspan') > 1) return;	//colspan 세팅시 리턴
...
AGrid.prototype._colculResizeWidth = function(inx, newLeft)
{
    ...
    */
	...
	let tmpInx = inx, tmp, isAllPercent = true, preLeft,
        headTds = this.hRowTmpl[0].children,
        headCols = this.headerTable.querySelectorAll('col'),
        tdEle = headTds[inx],
        beforeLeft = this.resizeBars[inx].beforeLeft,
        isLastColumn = inx == this.getColumnCount()-1;
    ...
    for(let i=headTds.length-1; i>-1; i--)
    {
        if(i<=inx && preLeft == undefined)
        {
            ...
            if(tmp && tmp.style.display != 'none')
            {
                ...
                preLeft = _TinyDom.position(tmp).left / this._dragScaleX - 4;
            }
        }
    ...
    if(preLeft == undefined) preLeft = - 4 + _TinyDom.position(this.showThead).left / this._dragScaleX;
	
    //숨겨져있는 경우가 있으므로 아래 내용으로 개선
	//if(inx>0) preLeft = _TinyDom.position(this.resizeBars[inx-1]).left;
	//컬럼이 숨겨져 있으면 resizeBar도 숨겨져있으므로 보이는 resizeBar를 찾는다.
	// while(tmpInx >= 0)
	// {
	// 	//tmpBar = this.resizeBars[--tmpInx];
    //     tmpBar = headTds[tmpInx--];
	// 	if(tmpBar && tmpBar.style.display != 'none')
	// 	{
    //         //newLeft 도 scale 전의 값이므로 이전 리사이즈바 위치도 scale 전의 값으로 가져온다.
	// 		preLeft = _TinyDom.position(tmpBar).left / this._dragScaleX - 4;
	// 		break;
	// 	}
	// }
    ...
    if(!isLastColumn || !isAllPercent) newLeft = Math.max(newLeft, _TinyDom.position(tdEle).left-4);
    
    let moveWidth = newLeft - preLeft,
        changedWidth, unit = '%';
    ...
    {
        //병합된 값을 가져온다.
        const colspan = this.getHeaderCell(0, inx).colSpan;

        //let val = this.headerTable.find('col')[inx].getAttribute('width');
        let val = headCols[inx].getAttribute('width');
        ...
        if(!val || val.indexOf('%')>-1)
        {
            ...
            moveWidth = moveWidth/_TinyDom.width(this.showThead) * 100; //moveWidth = moveWidth/this.headerTable.width() * 100;
