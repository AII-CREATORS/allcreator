...
class ASelectBox extends AComponent
{
    ...
    changeState(addState, remState)
    {
        if(remState) 
        {
            ...
            if(val) _TinyDom.removeClass(this.element, val);
        }
        ...
        if(addState) 
        {
            ...
            if(val) _TinyDom.addClass(this.element, val);
        }
    }

    _getDataStyleObj()
    {
        const ret = super._getDataStyleObj();
            
        const keyArr = ['data-style-focus', 'data-style-disable']
