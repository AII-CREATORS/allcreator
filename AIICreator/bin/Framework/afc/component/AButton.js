...
AButton.prototype.init = function(context, evtListener)
{
	...
	this.setOption(
	{
		imgAfterText: this.getAttr('data-aftertext'),	//이미지가 텍스트 뒤로 갈지 여부
		imgNewLine: this.getAttr('data-newline'),		//버튼과 이미지 줄바꿈 여부
...
AButton.prototype.setText = function(text)
{
    ...
    */
	...
	if(this.img)
	{
		if(this.option.imgAfterText) this.element.append(this.img);
		else this.element.prepend(this.img);
	}
...
AButton.prototype.setImage = function(url)
{
    ...
    */
	...
	if(url) 
	{
		...
		{
			if(this.option.imgNewLine) 
                this.element.prepend(document.createElement('br'));
	...
	else 
	{
		...
		this.element.removeAttribute('data-aftertext');
		this.element.removeAttribute('data-newline');
...
AButton.prototype.setBtnStyles = function(styles)
{
    this.clearStateClass();

    this.defStyle = styles[0];
    for(let i=0; i<AButton.STATE.length; i++)
		this.btnStyles[i] = styles[i+1]
...
AButton.prototype.downState = function()
{
	if(this.option.isToolBtn) 
	{
		...
		this.setStyle('background-position', -1*rt.width + 'px 0px');
	}
...
AButton.prototype.overState = function()
{
	...
	{
		var rt = this.getBoundRect();
		this.setStyle('background-position', -2*rt.width + 'px 0px');
	}
...
AButton.prototype.disableState = function()
{
	...
	{
		var rt = this.getBoundRect();
		this.setStyle('background-position', -3*rt.width + 'px 0px');
	}
...
AButton.prototype._getLastBgColor = function(ele)
{
    ...
    let color = _TinyDom.css(ele, 'background-color')

	//html 태그까지 올라가면 중단한다.
	if(ele.tagName=='HTML') color = 'rgb(255,255,255)';
	
	if(color=='transparent') 
		return this._getLastBgColor(ele.parentElement);
...
{
	const rgbArr = this._getLastBgColor(this.element), alpha = '';

	if(rgbArr.length==4) alpha = ',' + rgbArr[3];
	
	const hslArr = AUtil.RgbToHsl(rgbArr[0], rgbArr[1], rgbArr[2]);
	...
	const hslVal = 'hsl(' + hslArr[0] + ',' + hslArr[1] + '%,' + hslArr[2] + '%' + alpha + ')';
	//console.log(hslVal);
...
AButton.prototype.setIconMargin = function(value)
{
    ...
    if(this.img) this.img.style['margin'] = value;
};
...
AButton.prototype.setIconSize = function(value)
{
    ...
    */
	if(this.img) 
	{
		...
		this.img.style.width = value[0] || "auto";
		this.img.style.height = value[1] || "auto";
		
		if(this.img.style.width == "auto") this.img.style.width = ""
		if(this.img.style.height == "auto") this.img.style.height = ""
	}
};
...
AButton.prototype.getIconSize = function()
{
    ...
    */
	...
	if(this.img)
	{	
		retVal = this.img.style.width
		retVal += this.img.style.height ? (' ' + this.img.style.height) : ''
	}
...
AButton.prototype._getDataStyleObj = function()
{
	...
	let keyArr = ['data-style-over', 'data-style-down', 'data-style-disable'], val;
