...
afc.import("Framework/afc/component/ALabel.js")
afc.import("Framework/afc/component/AFlexLayout.js")
...
AMessageBox = class AMessageBox extends ADialog
{
    ...
    openBox(parent, title, message, type, callback)
    {
		...
		view.init()
		view.setStyleObj({ width: '260px', height: 'auto', 'background-color': 'rgb(76, 76, 76)', border: '0px solid rgb(255, 255, 255)',
			'font-size': '12px', padding: '20px', overflow: 'hidden', position:'relative'
		});

		this.lbl = new ALabel()
		this.lbl.init()
		this.lbl.setStyleObj({position:'relative', width: '100%', color: 'rgb(255, 255, 255)'})
		view.addComponent(this.lbl)

		this.btnLayout = new AFlexLayout() 
		this.btnLayout.init()
		this.btnLayout.setStyleObj({position:'relative', width: '100%', height: 'auto', 'margin-top': '30px', 'align-items': 'flex-start', 
			'justify-content': 'center', 'flex-wrap': 'wrap'
		})
		view.addComponent(this.btnLayout)

		return super.openDialog(view, parent, title)
...
AMessageBox.prototype.onCreate = function()
{
	...
	this.lbl.setHtml(this.message);
	this.lbl.setStyle('word-break', 'break-all');
	
	//this.btnLayout = comps[1];
...
AMessageBox.prototype._makeButton = function(text, value)
{
	...
	btn.setStyleObj({ width:'auto', height:'25px', 'margin-right':'5px', 'min-width':'50px' });
	btn.defaultBtnState();
