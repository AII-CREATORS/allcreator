...
class AListView extends AComponent
{
    ...
    {
        const selItem = this.element.querySelector('.AListView-select')
        
        if(state=='select') 
        {
            if(styleNames) 
            {
                //스타일 지정 안 했을 때.. 추가했던 클래스 제거
                selItem.classList.remove('listview-select-style')
            }

            //아무 스타일도 지정하지 않았으면 기본 스타일 추가
            else styleNames = 'listview-select-style'

            selItem.classList.add(...styleNames.trim().split(' '))
            selItem.setAttribute('data-cur-applys', styleNames)

            //스타일 추가한 후 리턴
            return
        }

        const curStyles = selItem.getAttribute('data-cur-applys')
        if(curStyles) 
        {
            selItem.classList.remove(...curStyles.trim().split(' '))
            selItem.removeAttribute('data-cur-applys')
        }
    } 
    */   
...
AListView.prototype.beforeInit = function()
{
	...
	this.scrollArea.style.cssText = 'position:relative; z-index:0; width:100%; height:100%; overflow:auto; -webkit-overflow-scrolling:touch;';

	//------------------------------------------------------------------

	this.itemWrapper = document.createElement('div');
	this.itemWrapper.className = 'list_item_wrapper';
	...
	*/
	this.itemWrapper.style.display = 'flex';
	this.itemWrapper.style.display = '-ms-flexbox';

	const gap = this.getAttr('item-gap');
	if(gap) this.itemWrapper.style.gap = gap;

	this.scrollArea.appendChild(this.itemWrapper);
};
...
AListView.prototype.init = function(context, evtListener)
{
	...
	*/
    ...
    //this.dividerColor = this.getAttr('data-color-divider')
    this.itemClass = this.getAttr('data-style-item')
	
	var items = this.element.children;

	if(this.option.direction=='horizontal')
	{
		Object.assign(this.itemWrapper.style,
		{
			'flex-direction': 'row',
            'align-items': 'stretch',
	...
	var selClass = this.getAttr('data-style-select', 'AListView-select');
	...
	if(this.isDev())
	{
		this.element.classList.add('listview-dev');
	}
	//실행 시점
	else
	{
		...
		for(const row of this.element.querySelectorAll(':scope > .listview-row')) row.remove();
		this.element.prepend(this.scrollArea);
...
AListView.prototype.enableScrlManager = function()
{
	...
	this.scrollArea.style.setProperty('-webkit-overflow-scrolling', '');
...
AListView.prototype.scrollImplement = function()
{
	...
	var scrlArea = this.scrollArea, transTop;
...
AListView.prototype.setScrollArrow = function(topHeight)
{
	...
	sa.apply(this.scrollArea);

	if(topHeight) sa.arrow1.style.top = topHeight+'px';
};
...
AListView.prototype._enableScrollIndicator = function()
{
	...
	this.scrlIndicator.init(type, this.scrollArea);
};
...
AListView.prototype.setSelectItem = function(item)
{
    if(this.selectItem)
    {
        ...
        if(this.selectClass) this.selectItem.classList.remove(this.selectClass);
    ...
    if(this.selectItem)
    {
        ...
        if(this.selectClass) this.selectItem.classList.add(this.selectClass);
...
{
	return Array.from(this.getItems()).indexOf(item);
};
...
AListView.prototype.removeAllItems = function()
{
    ...
    {
		//async 로 addItem이 되고 view가 item이 세팅되기전에 제거되는 경우의 체크
    	if(el.view) el.view.removeFromView();
...
{
	_TinyDom.hide(this.scrollArea);
	var thisObj = this;
	setTimeout(function(){ _TinyDom.show(thisObj.scrollArea); }, 1);
};
...
AListView.prototype.scrollTo = function(pos, isAni)
{
	if(isAni)
	{
		if(this.option.direction=='horizontal') this.scrollArea.scrollTo({left: pos, behavior: 'smooth'});
		else this.scrollArea.scrollTo({top: pos, behavior: 'smooth'});
	}
	else
	{
		if(this.option.direction=='horizontal') this.scrollArea.scrollLeft = pos;
		else this.scrollArea.scrollTop = pos;
	}
};
...
{
	var scrlVal = 0;
	if(this.option.direction=='horizontal') scrlVal = this.scrollArea.scrollLeft + offset;
	else scrlVal = this.scrollArea.scrollTop + offset;
...
{
    //this.scrollArea.scrollTop = this.scrollArea.scrollHeight*-1;
...
AListView.prototype.scrollToBottom = function(isAni)
{
	...
	this.scrollTo(this.scrollArea.scrollHeight, isAni);
};
...
{
    var thisObj = this, item, aview, newItems = [];
    ...
    for(var i=0; i<dataLen; i++)
    {
        ...
        item.className = 'listview-row';

        if(this.itemClass) item.classList.add(this.itemClass)
...
AListView.prototype.updatePosition = function(pWidth, pHeight)
{
    ...
    if(isUpdate || this.option.isCheckUpdatePos)
    {
    	for(const el of this.getItems())
    	{
        	if(el.view && (isUpdate || el.view.isUpdatePosFromListView) )
				el.view.updatePosition();
    	}
    }
};
...
AListView.prototype.itemInsertManage = function(items, posItem, isPrepend, isOuterItem)
{
	...
	if(isOuterItem)
	{
		...
		{
			if(el.view) el.view.owner = this;
		}
	}

	if(posItem)
	{
		var arrItems2 = Array.isArray(items) ? items : [items];
		if(isPrepend) for(const el of arrItems2) posItem.before(el);
		else for(const el of arrItems2) posItem.after(el);
	}
	else
	{
		if(isPrepend)
		{
			...
			this.itemWrapper.prepend(...(Array.isArray(items) ? items : [items]));
		}
		else
		{
			...
			this.itemWrapper.append(...(Array.isArray(items) ? items : [items]));
		}
	}
};
...
{
	if(this.scrollArea.scrollLeft > 0) return true;
	else return false;	
};

AListView.prototype.isMoreScrollRight = function()
{
	if(this.scrollArea.offsetWidth + this.scrollArea.scrollLeft < this.scrollArea.scrollWidth) return true;
	else return false;	
};

AListView.prototype.isScroll = function()
{
    return (this.scrollArea.offsetHeight < this.scrollArea.scrollHeight) || (this.scrollArea.offsetWidth < this.scrollArea.scrollWidth);
};
...
AListView.prototype.createBackup = function(maxRow, restoreCount)
{
	...
	//this.bkManager.setBackupInfo(this.itemHeight, 1, this.scrollArea, this.scrollArea);

    //백업매니저를 사용할 경우는 명확히 아이템 사이즈를 지정해 줘야 한다.(ex, '60px')
	this.bkManager.setBackupInfo(parseInt(this.itemHeight), 1, this.scrollArea, this.itemWrapper);
...
AListView.prototype._getDataStyleObj = function()
{
	...
	let keyArr = ['data-style-item', 'data-style-select'], val;
...
AListView.prototype._setDataStyleObj = function(styleObj)
{
	...
	{
		if(p==afc.ATTR_STYLE) this._set_class_helper(this.element, null, styleObj, p);
		else if(p=='data-style-select') this._set_class_helper(this.element, this.element.querySelectorAll('.AListView-select'), styleObj, p);
        else if(p=='data-style-item') this._set_class_helper(this.element, this.element.querySelectorAll('.listview-row'), styleObj, p);
	}
};
