...
ANavigator.prototype.flipPage = function(willActivePage, isFirst)
{
	...
	if(this.flipType=='normal')
	{
		...
		else Object.assign(willActivePage.element.style, {'visibility': 'visible', 'height':'100%'});
		...
		if(willDeactivePage)
		{
			...
			else Object.assign(willDeactivePage.element.style, {'visibility': 'hidden', 'height': '0px'});
	...
	else if(this.flipType=='slide')
	{
		...
		willActivePage.element.classList.add('slide-in-'+this.slideDir);
		willActivePage.onActive(isFirst);

		if(willDeactivePage)
		{
			willDeactivePage.element.classList.add('slide-out-'+this.slideDir);
			willDeactivePage.onDeactive();
		}

		willActivePage.element.addEventListener('webkitAnimationEnd', function()
		{
			if(willDeactivePage)
			{
				willDeactivePage.element.classList.remove('slide-out-'+thisObj.slideDir);
				//willDeactivePage.element.style.display = 'none';
			...
			willActivePage.element.classList.remove('slide-in-'+thisObj.slideDir);
...
ANavigator.prototype.goPage = async function(pageId, data, isNoHistory)
{
	...
	if(!pageInfo.pageObj)
	{
		...
		if(!this.isDeactiveGone) pageInfo.pageObj.element.style.overflow = 'hidden';
