...
AListViewAttrProp.prototype.init = function(context, evtListener)
{
    ...
    // this.setAttrPath('afc', 'AListView')
    ...
    this.makeAttrItem('afc', 'AListView').then(()=>{
        this.itemInfoGrid = this.findCompByClass('AGrid')[0];
    })
};
...
AListViewAttrProp.prototype.applyValueToSelComp = function(selComp, dataKey, valGroup, value)
{
	...
	else if(valGroup=='CSS_VALUE')
	{
		...
		{
			var items = selComp.element.children,
				preVal = items[0].style.height;
...
AListViewAttrProp.prototype.getUpdateValue = function(selComp, dataKey, groupName)
{
	...
	if(this.selCompArr.length==1)
	{
		if(groupName=='CSS_VALUE')
		{
			if(dataKey=='item-height')
			{
				return selComp.element.firstElementChild.style.height;
			}
		}
	}
...
AListViewAttrProp.prototype.updateAGrid = function(dataKey, valueComp, value)
{
...
AListViewAttrProp.prototype.openUrlInputDlg = function(comp, title, inputText)
{
	...
	wnd.open('Source/popup/InputDlg.lay', null, title, ofs.left, ofs.top+compH, w, '174px').then(() => {
...
AListViewAttrProp.prototype.onAddItemBtnClick = function(comp, info, e)
{
	...
	wnd.setResultCallback(function(result, data)
	{
		if(result==0 && data)
		{
			...
			if(rowCount==0)
			{
				listView.element.querySelectorAll('.listview-row').forEach(el => el.remove());
			}

			listView.element.insertAdjacentHTML('beforeend', '<div class="listview-row"></div>');
...
AListViewAttrProp.prototype.onDeleteItemBtnClick = function(comp, info, e)
{
	...
	if(selRows.length>0) 
	{
		...
		var listRows = listView.element.querySelectorAll('.listview-row');
		if(listRows[selIndex]) listRows[selIndex].remove();
		...
		if(rowCount==0)
		{
			...
			listView.element.insertAdjacentHTML('beforeend', '<div class="listview-row" style="height: 60px;"></div><div class="listview-row AListView-select" style="height: 60px;"></div>\
...
AListViewAttrProp.prototype.applyGridChange = function()
{
	...
	{
		url = thisObj.itemInfoGrid.getCellText(row, 0);
	...
	*/
	for(let i=0; i<rows.length; i++)
	{
		...
		value[data] = [ i, url, data ];

		thisObj.applyUrlToItem(selComp, i, url);
	}	
...
AListViewAttrProp.prototype.applyUrlToItem = function(listView, inx, url)
{
	...
	if(item)
	{
		...
		var listItem = listView.element.querySelectorAll('.listview-row')[inx];
		if(!listItem) return;
		...
		if(html)
		{
			...
			listItem.querySelectorAll('.RGrid-Style').forEach(el => { el.removeAttribute('id'); Object.assign(el.style, {border: '1px solid blue', textAlign: 'center'}); el.textContent = 'rMate Grid'; });
			listItem.querySelectorAll('.RChart-Style').forEach(el => { el.removeAttribute('id'); Object.assign(el.style, {border: '1px solid yellow', textAlign: 'center'}); el.textContent = 'rMate Chart'; });

			for(const child of listItem.children) child.style.position = 'relative';
		}
	}
