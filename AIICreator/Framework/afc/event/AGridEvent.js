...
AGridEvent.prototype._refresh = function()
{
	...
	ri.init(agrid, agrid.scrollArea);
...
AGridEvent.prototype._scroll = function()
{
	...
	var agrid = this.acomp, oldScrollTop = agrid.scrollArea.scrollTop;

	agrid.scrollArea.addEventListener('scroll', function(e)
	{
...
AGridEvent.prototype._select = function()
{
	...
	AEvent.bindEvent(ele, AEvent.ACTION_UP, function(e)
	{
		...
		{
			let inx = agrid.rowIndexOfCell(selItem);
			//console.log(inx);
		...
		if(e.which == 3)
		{
            ...
            if(!evtEles[0].classList.contains(agrid.selectStyleName)) agrid.selectCell(evtEles, e);
		}
		else
		{
...
{
	//var thisObj = this;
	let thisObj = this, agrid = this.acomp, timeout = null, isTouchLeave = true,
