...
ADropBoxEvent.prototype._change = function()
{
    ...
    adropbox.element.addEventListener('keyup', (e)=>
	{
...
ADropBoxEvent.prototype._select = function(itemList)
{
    ...
    const adropbox = this.acomp;
    let isTouchLeave = true, startX = 0, startY = 0, focusEle;
    
    AEvent.bindEvent(itemList, AEvent.ACTION_DOWN, (e)=>
    {
        isTouchLeave = false;
    ...
    AEvent.bindEvent(itemList, AEvent.ACTION_MOVE, (e)=>
    {
        const oe = e.changedTouches[0];
        ...
        focusEle = adropbox.scrollArea.querySelector('.'+adropbox.focusClass);
		
		if(focusEle != itemList)
		{
			focusEle?.classList.remove(adropbox.focusClass);
			itemList.classList.add(adropbox.focusClass);
		}
		
        if(Math.abs(oe.clientX - startX) > AEvent.TOUCHLEAVE || Math.abs(oe.clientY - startY) > AEvent.TOUCHLEAVE)
        {
			//$(itemList).removeClass(adropbox.focusClass);
            isTouchLeave = true;
        }
    });

    AEvent.bindEvent(itemList, AEvent.ACTION_UP, (e)=>
    {        
        ...
        */
		itemList.classList.remove(adropbox.normalClass);
		itemList.classList.add(adropbox.selectClass);
        adropbox.selectItem(itemList.index);
		...
		{ 
			...
			if(!adropbox.isReadOnly)
			{
				const len = adropbox.getEditText().length;
				adropbox.textfield.focus();					
				adropbox.textfield.setSelectionRange(len, len);	// 마우스로 선택 후 커서위치 조정
			}
			adropbox.reportEvent('select', itemList);
    ...
    AEvent.bindEvent(itemList, AEvent.ACTION_CANCEL, (e)=>
    {        
        isTouchLeave = true;
    });
	
    AEvent.bindEvent(itemList, 'mouseleave', (e)=>
    {
		itemList.classList.remove(adropbox.focusClass);
    });
};
