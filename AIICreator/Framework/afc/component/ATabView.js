...
ATabView.CONTEXT = 
{
    tag: '<div data-base="ATabView" data-class="ATabView" class="ATabView-Style" data-state="0" >'+
	        '<div class="tab_area">'+
	            '<span class="ATabView-select">tab1</span>'+
	            '<span class="ATabView-deselect">tab2</span>'+
	            '<span class="ATabView-deselect">tab3</span>'+
			'</div>'+
...
ATabView.prototype.init = function(context, evtListener)
{
	...
	this.tabArea = this.element.querySelector('.tab_area');
	this.tabContents = this.element.querySelector('.tab_contents');
	this.tabBtnTpl = this.tabArea.children[1].cloneNode(true);
	...
	var styles = [this.getAttr('data-style-select'), this.getAttr('data-style-tab')];
	...
	if(!this.isDev())
	{
		for(const ch of [...this.tabArea.children]) ch.remove();
		...
		if(tabInfos)
		{
			...
			{
				var selTabId = thisObj.element.getAttribute('data-tab-select');
	...
	{
		let selTab = this.tabArea.querySelector('.ATabView-select')
		if(selTab) _TinyDom.addClass(selTab, styles[0])

		let selTabs = this.tabArea.querySelectorAll('.ATabView-deselect')
		selTabs.forEach(tab=> { _TinyDom.addClass(tab, styles[1]) })
	}
...
ATabView.prototype.setOption = function(option, noOverwrite)
{
	...
	{
		for(const el of this.getAllTabs())
    	{
			if(!el.content) break

        	Object.assign(el.content.style,
			{
				display: 'none',
				visibility: '',
				height: '100%'
...
{
	var tabHeight = _TinyDom.height(this.tabArea);

    _TinyDom.show(this.tabArea);
	this.tabContents.style.height = 'calc(100% - '+ tabHeight +'px)';
};

ATabView.prototype.hideTabArea = function()
{
	_TinyDom.hide(this.tabArea);
	this.tabContents.style.height = '100%';
};
...
{
    //탭버튼 템플릿을 복사하여 추가한다.
   	var tabEle = this.tabBtnTpl.cloneNode(true);
   	tabEle.textContent = name;

    this.tabArea.appendChild(tabEle);
	...
	if(this.option.deactiveGone) content.style.display = 'none';
	else Object.assign(content.style, {'visibility': 'hidden', 'height': '0px'});
	...
	{
		AView.setViewInItem(url, content, this);
		//tabEle.view = url;
    ...
    if(tabEle.tabId) content.setAttribute('id', this.element.id+'-'+tabEle.tabId);
	...
	*/
	...
	if(this.btnStyles[1]) _TinyDom.addClass(tabEle, this.btnStyles[1])
...
ATabView.prototype.getTabById = function(tabId)
{
    ...
    for(const el of this.tabArea.children)
    {
        if(el.tabId==tabId)
        {
            retTab = el;
            break;   //each callback 리턴
        }
    }
...
{
    var retTab = null;
    if(index<0) retTab = this.tabArea.lastElementChild;
    else retTab = this.tabArea.children[index];
...
ATabView.prototype._tabChangeManage = async function(tabEle)
{
	...
	if(this.oldTab) 
	{
		...
		*/
		...
		if(this.btnStyles[0]) _TinyDom.removeClass(this.oldTab, this.btnStyles[0]) //this.oldTab.classList.remove(this.btnStyles[0]);
		this.oldTab.classList.remove('ATabView-select');

		if(this.btnStyles[1]) _TinyDom.addClass(this.oldTab, this.btnStyles[1]) //this.oldTab.classList.add(this.btnStyles[1]);
		this.oldTab.classList.add('ATabView-deselect');
	}
	...
	*/
	...
	if(this.btnStyles[1]) _TinyDom.removeClass(this.selectedTab, this.btnStyles[1])//this.selectedTab.classList.remove(this.btnStyles[1]);
	this.selectedTab.classList.remove('ATabView-deselect');
	if(this.btnStyles[0]) _TinyDom.addClass(this.selectedTab, this.btnStyles[0])//this.selectedTab.classList.add(this.btnStyles[0]);
	this.selectedTab.classList.add('ATabView-select');
...
ATabView.prototype.checkMaxLoadedTabs = function() 
{
	...
	for(const el of this.getAllTabs())
	{
		if(el.content.view)
		{
			...
			if(el._activeTime < minVal)
			{
				minVal = el._activeTime;
				remTab = el;
			}
		}
	}
...
ATabView.prototype.activeTab = function(oldTab, newTab, reload) 
{
	...
	if(this.option.enableAnimation)
	{
		switch(this.option.changeAnimation)
		{
			...
			{
				if(oldTab)
					oldTab.content.classList.add('slide-out-'+this.option.slideDir);

				let newContent = newTab.content;
	           	//newContent.show();
				if(this.option.deactiveGone) _TinyDom.show(newContent);
				else Object.assign(newContent.style, {'visibility': 'visible', 'height':'100%'});

				newContent.classList.add('slide-in-'+this.option.slideDir);

	           	newContent.addEventListener('webkitAnimationEnd', function()
	           	{
	        		if(oldTab)
	        		{
	        			oldTab.content.classList.remove('slide-out-'+thisObj.option.slideDir);
	        			//$(oldTab.content).hide();
						if(thisObj.option.deactiveGone) _TinyDom.hide(oldTab.content);
						else Object.assign(oldTab.content.style, {'visibility': 'hidden', 'height': '0px'});
	        		}

	            	newContent.classList.remove('slide-in-'+thisObj.option.slideDir);
			...
			case 'fade':
			{
				newTab.content.style.display = ''; if(_effectCallback) _effectCallback();

				if(oldTab) _TinyDom.hide(oldTab.content);
			}
			break;

			default:
			{
				_TinyDom.show(newTab.content); if(_effectCallback) _effectCallback();
	...
	else
	{
		...
		if(oldTab)
		{
			...
			if(this.option.deactiveGone) _TinyDom.hide(oldTab.content);
			else Object.assign(oldTab.content.style, {'visibility': 'hidden', 'height': '0px'});
		}
...
ATabView.prototype._getDataStyleObj = function()
{
	...
	var keyArr = ['data-style-tab', 'data-style-select'], val;
...
ATabView.prototype._setDataStyleObj = function(styleObj)
{
	for(var p in styleObj)
	{
		...
		else if(p=='data-style-tab') this._set_class_helper(this.element, this.element.querySelectorAll('.ATabView-deselect'), styleObj, p);
		else if(p=='data-style-select') this._set_class_helper(this.element, this.element.querySelectorAll('.ATabView-select'), styleObj, p);
	}
};
