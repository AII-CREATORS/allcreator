...
class ADataGrid extends AView
{
    ...
    autoSplitAddData(rowData, metaData, noUpdate)
    {
        if(this.rowsetCount>1)
        {
            const size = parseInt(rowData.length/this.rowsetCount);
                
            const result = Array.from({ length: this.rowsetCount }, (_, i) =>
                rowData.slice(i * size, (i + 1) * size)
            );

            this.addRowDataArr(result, [metaData], noUpdate)
        }
        else this.addRowData(rowData, metaData, noUpdate)
    }    
    ...
    setRowsetCount(count)
    {
        this.rowsetCount = parseInt(count) || 1;
    }
    ...
    {
        if(this.pivotGrid) this.pivotGrid.devApplyStateStyles(state, styleNames)
        this.grid.devApplyStateStyles(state, styleNames)
    }
...
ADataGrid.prototype.init = function(context, evtListener)
{
	...
	this.setOption(
	{
        ...
        isColumnChange: true,
        isSameSelectEvent: true                                         //선택된 셀을 다시 선택할 때, select 이벤트 발생 여부
    ...
    this.setRowsetCount(this.getAttr('data-rowset-count'));
	...
	if(this.pivotGrid) 
	{
        ...
        this.pivotGrid.scrollArea.style['overflow-y'] = 'hidden';

        //this.$pTmplCells = this.pivotGrid.$rowTmpl.eq(0).children('td');
        this.pTmplCells = this.pivotGrid.rowTmpl[0].children;
	}

    //마스크 관련 작업을 위해 미리 얻어둔다.
    //this.$tmplCells = this.grid.$rowTmpl.eq(0).children('td');
    this.tmplCells = this.grid.rowTmpl[0].children;
	
    this.aevent.applyEvent()
	
    //기본 스크롤바가 생기지 않도록
	this.grid.scrollArea.style['overflow-y'] = 'hidden';
	...
	this.scrollBarV.setScrollArea(_TinyDom.height(this.grid.scrollArea), this.grid.hRowTmplHeight, this.grid.rowTmplHeight);
...
ADataGrid.prototype._regDragManage = function(isReg)
{
	...
	for(i=0; i<this.grid.hRowTmpl.length; i++)
	{
        ...
        //$cells.each((inx, cell)=>
        cells = this.grid.hRowTmpl[i].children
...
{
	const dropEle = e.currentTarget;	//드랍한 element

    const fromInx = this.grid.colIndexOfCell(dragEle),
...
ADataGrid.prototype.moveColumn = function(fromInx, toInx, noUpdate)
{
    ...
    let totColSpan = this.grid.hRowTmpl[0].children[fromInx].colSpan
    ...
    function _move_helper(tmpl, table)
    {
        ...
        cells = table.querySelectorAll('col')
        toEle = cells[toInx]

        for(i=0; i<totColSpan; i++)
        {
            //$toEle.before($cells.eq(fromInx+i))
            toEle.before(cells[fromInx+i])
        }
...
ADataGrid.prototype.getAllColumnName = function()
{
    ...
    if(this.pivotGrid)
    {
        tmplCells = this.pivotGrid.hRowTmpl[0].children
        for(const ele of tmplCells) //.each((i, ele) =>
        {
            retArr.push(ele.innerText)
        }
    }

    tmplCells = this.grid.hRowTmpl[0].children
    for(const ele of tmplCells) //.each((i, ele) =>
    {
        retArr.push(ele.innerText)
    }
...
{
    let scrlSize = _TinyDom.width(this.scrlView.element)
    ...
    if(scrlSize)
    {
        ...
        let contentSize = _TinyDom.width(this.grid.element)+pivotAdd

        this.scrollBarH.setScrollArea(scrlSize, 0, 1)
        this.scrollBarH.setDataCount(contentSize)

        if(!this.option.isHideHScrollbar)
        {
            //스크롤이 가능한 경우에만 가로 스크롤바를 표시한다. (불가능하면 스크롤바 자체를 숨김)
            if(contentSize - scrlSize > 1) this.scrollBarH.show()
            else this.scrollBarH.hide()
        }
    }
};
...
ADataGrid.prototype.updatePosition = function(width, height)
{
    ...
    let scrlSize = _TinyDom.height(this.grid.scrollArea);
...
ADataGrid.prototype.clearSelected = function()
{
    ...
    //이벤트 객체의 선택 영역도 초기화해준다.
    this.aevent.clearEvtObj();
};
...
{
	let inx = this.selObjs.indexOf(selObj);
	
	if(inx>-1) 
	{
        if(this.aevent.evtObj.rowInx == this.showArr2.indexOf(selObj.parentDataArr))
        {
            if(this.option.isFullRowSelect) this.aevent.clearEvtObj()
            else if(this.aevent.evtObj.colInx == selObj.parentDataArr.indexOf(selObj)) this.aevent.clearEvtObj()
        }
...
ADataGrid.prototype._gridClickManage = function(acomp, info, e)
{
    ...
    let	oriRowInx = this.dataInx + pos[0],
        rowInx = parseInt(oriRowInx/this.rowsetCount) * this.rowsetCount,
	...
	{
		let isClear = (this.option.isSingleSelect || !e.ctrlKey);	//싱글 셀렉트가 아니고 컨트롤키가 눌리면 
		...
		{
			let colCnt = this.grid.columnCount;
			
			if(this.pivotGrid) colCnt += this.pivotGrid.columnCount;
			
            evtObj.selObj = [];

            for(let r=0; r<this.rowsetCount; r++)
            {
                parentDataArr = this.showArr2[rowInx + r];
                ...
                evtObj.selObj.push(parentDataArr);
            }

            if(this.rowsetCount == 1) evtObj.selObj = evtObj.selObj[0];
...
{
	let areaCnt = this.scrollBarV.getCountPerArea();
	let renderCnt = Math.min(areaCnt, this.showArr2.length);
	...
	if(isReset)
	{
		...
		if(this.scrollMoveState)
		{
			...
			if(this.scrollMoveState==1)
			{
                ...
                if(this.pivotGrid)
                {
                    for(const child of this.pivotGrid.tBody.children)
                        _TinyDom.hide(child);
                }

                for(const child of this.grid.tBody.children)
                    _TinyDom.hide(child);
			}
			else 
			{
                ...
                {
                    for(const child of [...this.pivotGrid.tBody.children])
                        if(child.checkVisibility()) child.remove()
                }
...
{
	let end = this.dataInx + this.renderRowCnt, 
		i = this.dataInx, j = 0;
	...
	let dataOffset = this.getPivotColumnCount()
...
ADataGrid.prototype.enableScrlManagerY = function()
{
	...
	this.scrlManagerY.setStopCallback(()=>
	{
		//스크롤이 완료되면 값을 리셋한다.
		this.scrollMoveState = 0;
	
		//if(this.pivotGrid) this.pivotGrid.tBody.find(':hidden').remove();
		//this.grid.tBody.find(':hidden').remove();
...
ADataGrid.prototype.setGridData = function(dataArr2, updateType, isFiltered)
{
	...
	if(updateType != 2)
	{
        //선택되어있던 데이터들의 select 초기화를 위해 clearSelected로 변경
		//this.selObjs.length = 0;
        this.clearSelected();
...
ADataGrid.prototype.filter = function(filterFunc, updateType)
{
    ...
    {
        const rowsetCount = this.getRowsetCount();
        let arr;
        if(rowsetCount == 1) arr = this.dataArr2.filter(filterFunc);
        else
        {
            arr = [];
            let isFilterOK, len = this.dataArr2.length;
            for(let i=0; i<len; i+=rowsetCount) {
                isFilterOK = false;
                for(let j=0; j<rowsetCount; j++) {
                    if(filterFunc(this.dataArr2[i+j])) {
                        isFilterOK = true;
                        break;
                    }
                }
...
ADataGrid.prototype._maskRowData = function(rowData)
{
    ...
    for(i=0; i<rowData.length; i++)
    {
        ...
        {
            dataObj = rowData[i]
            if(!dataObj.style) dataObj.style = {};
...
ADataGrid.prototype._chgQueryToArr = function(queryData, keyArr)
{
    ...
    else if(typeof queryData[0] != 'object')
    {
        queryData.forEach((v, i) => queryData[i] = { text: v });
    }
...
ADataGrid.prototype.setCellText = function(rowInx, colInx, text, noUpdate)
{
    //number
    if(rowInx.length==undefined)
    {
        colInx = this._chgKeyToInx(colInx)

        const data = this.dataArr2[rowInx][colInx]
        data.text = text;

        //data 내의 text 원본을 바꾼다.
        this._maskCellData(colInx, data);
    }
    //row arr
    else
    {
        const data = rowInx[colInx]
        data.text = text;
        this._maskCellData(colInx, data);
    }
...
ADataGrid.prototype.setCellData = function(rowInx, colInx, cellData, noUpdate)
{
    ...
    else
    {
        ...
        rowInx[colInx] = this._maskCellData(colInx, cellData);
    }
...
ADataGrid.prototype.sortColumn = function(colInx)
{
    ...
    // sort Image 변경
    if(this.sortImg) this.sortImg.setAttribute('src', 'Framework/afc/image/' + sortImgName + '.png');
    else
    {
        ...
        if(cell)
        {
            ...
            this.sortImg.src = 'Framework/afc/image/' + sortImgName + '.png';

            this.sortImg.style.verticalAlign = 'middle';
            this.sortImg.style.marginLeft = '5px';
            this.sortImg.style.marginRight = '-21px';

            //$(cell).append(this.sortImg);
            cell.appendChild(this.sortImg);
        }
    }
...
//------------------------------------------------------------------------------------------------
	...
	{
		const textA = a[colInx].text;//.toUpperCase();
		const textB = b[colInx].text;//.toUpperCase();
...
ADataGrid.prototype.getSelectedRowArrs = function()
{
	...
	for(let i=0; i<this.selObjs.length; i++)
	{
...
ADataGrid.prototype.getRowCount = function()
{
	return this.showArr2.length;
};
...
{
	let mappingArr = this.grid.getMappingCount();
	if(this.pivotGrid)
	{
		let pMappingArr = this.pivotGrid.getMappingCount();
...
ADataGrid.prototype.regRowToRealMap = function(data, row)
{
...
ADataGrid.prototype.getRealMapRow = function(data)
{
    return this.realMap[this.getRealKey(data)];
};
...
{
	let data = dataArr[0], arr

    if(!keyArr) keyArr = this.oriColumnKeys
...
ADataGrid.prototype._setRowsByObj = function(grid, dataInx, rowInx, rowData, start, end, dataOffset)
{
	...
	let i, obj, cell, checked = '', childEle, label, tmp, selArr = [] ;
	for(i=start; i<=end; i++)
	{
		...
		if(obj.type)
		{
            ...
            else if(obj.type=='select')
            {
                ...
                childEle = cell.firstElementChild
		...
		if(obj.rowSpan && this.isResetRow)
		{
			grid.mergeRow(rowInx, i, obj.rowSpan);
		}
		
		if(obj.colSpan && this.isResetRow)
		{
        ...
        if(tmp != undefined) cell.style.cssText = tmp;
        ...
        //if(obj.select) grid._addCell(cell);
        if(obj.select) selArr.push(cell);
	}

    if(selArr.length>0) grid._addCell(selArr);
...
ADataGrid.CONTEXT = 
{
    tag: '<div class="ADataGrid-Style" data-base="ADataGrid" data-class="ADataGrid"\
		...
		<div data-base="AGrid" data-class="AGrid" class="AGrid-Style" data-ver="3302" data-selectable="true" style="width: 200%; height: 100%; left: 0px; top: 0px;">\
