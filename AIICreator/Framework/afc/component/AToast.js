...
AToast.single = function(style){
	AToast.globalToast = new AToast(style);
};

AToast.show = function(text, duration, style)
{
	var toast;
	if(AToast.globalToast) toast = AToast.globalToast;
	else toast = new AToast(style);
...
AToast.prototype.show = function(text, duration)
{
	...
	else
	{
		...
		if(this.style)
            _TinyDom.addClass(this.frame, this.style)
