...
ADockingFrame = class ADockingFrame extends AFrameWnd
{
	...
	{
		await super.open(viewUrl, parent, title, left, top, width, height)
...
ADockingFrame.readFramePosition = function(defPos) 
{
	...
	if(window.localStorage)
		strData = window.localStorage.getItem(theApp.getCurrentPath());
...
ADockingFrame.writeFramePosition = function() 
{
	...
	if(window.localStorage)
		window.localStorage.setItem(theApp.getCurrentPath(), JSON.stringify(ADockingFrame.framePosition, undefined, 4));
...
ADockingFrame.prototype._makeTitle = async function()
{
	...
	_TinyDom.hide(this.minBtn);
    _TinyDom.hide(this.maxBtn);
...
ADockingFrame.prototype.close = function(result)
{
	...
	else if(!this.oldInfo) ADockingFrame.setFramePosition(frmId, this.getPositionInfo());
