...
class APivotGrid extends AView
{
    ...
    devApplyStateStyles(state, styleNames)
    {
        ...
        this.scrollGrid.devApplyStateStyles(state, styleNames)
...
APivotGrid.prototype.init = function(context, evtListener)
{
	...
	else
	{
		...
		{
			this.pivotGrid.element.style.position = 'relative';
			this.scrollGrid.element.style.position = 'relative';
		}
	}
	...
	this.scrollGrid.scrollArea.style.overflowY = 'visible';	//or hidden
	...
	if(!window.APivotGridEx || !(this instanceof APivotGridEx))
	{
		...
		if(this.element.style.height && this.element.style.height!='auto')
		{
			...
			this.pivotGrid.enableScrlManager(null, this.scrollGrid.scrollArea);
			this.scrollGrid.enableScrlManager(this.pivotGrid.scrollArea, null);
...
APivotGrid.prototype._initEx = function()
{
	...
	if(!this.scrlYView)
	{
		...
		this.scrlYView.init();
		Object.assign(this.scrlYView.element.style, {
			overflow: 'auto',
			left: '0px',
			top: '0px',
			width: '100%',
		...
		Object.assign(this.pivotGrid.element.style, {
			overflow: 'visible',
			height: 'auto',
			position: 'sticky',
			left: 0
		...
		this.pivotGrid.element.append(...[...this.pivotGrid.scrollArea.children]);
		this.pivotGrid.scrollArea.remove();

		Object.assign(this.scrollGrid.element.style, {
			overflow: 'visible',
			height: 'auto'
		});
		this.scrollGrid.element.append(...[...this.scrollGrid.scrollArea.children]);
		this.scrollGrid.scrollArea.remove();


		Object.assign(this.scrollView.element.style,
			{
				overflow: 'visible',
				'background-color': 'transparent'
			});
...
{
	const pivotTR = this.pivotGrid.showThead.children;
	const scrollTR = this.scrollGrid.showThead.children;

	for(const tr of pivotTR) {
		tr.style.height = '';
	}
	for(const tr of scrollTR) {
		tr.style.height = '';
	}
	...
	{
		maxHeight = 0;
		for(const td of pivotTR[i].children) {
		...
		for(const td of scrollTR[i].children) {
			rowspan = td.getAttribute('rowspan');
			if(!rowspan)
			{
				maxHeightArr[i] = Math.max(td.offsetHeight, maxHeightArr[i]);
	...
	for(let i=0; i < maxCnt; i++)
	{
		pivotTR[i].style.height = maxHeightArr[i] + 'px';
		scrollTR[i].style.height = maxHeightArr[i] + 'px';
	}
};
...
APivotGrid.prototype._setDataStyleObj = function(styleObj)
{
    ...
    //data-style 값이 있으면 default style 이므로 뷰에도 스타일을 셋팅한다.
    if(styleObj[afc.ATTR_STYLE]!=undefined)
    {
        AView.prototype._setDataStyleObj.call(this, styleObj)
    }
...
APivotGrid.prototype._onGridActionMove = function(comp, info, e)
{
	...
	if(Math.abs(this.startX - curX) > 10)
	{
		...
		this.scrlYView.element.style.overflowY = 'hidden';
	}
	else if(Math.abs(this.startY - curY) > 10)
	{
		this.axisScrollLock = true;
		this.scrlYView.element.style.overflowX = 'hidden';
	}
};
...
{
	this.axisScrollLock = false;
	this.scrlYView.element.style.overflowY = 'auto';
	this.scrlYView.element.style.overflowX = 'auto';
};
