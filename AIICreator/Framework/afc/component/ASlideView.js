...
ASlideView.prototype.init = function(context, evtListener)
{
	...
	if(this.option.direction=='vertical') this.element.style.whiteSpace = 'normal';
	else this.element.style.whiteSpace = 'nowrap';
};
...
ASlideView.prototype.enableScrlManager = function()
{
	...
	this.element.style.zIndex = 0;	//가속을 위한 z-index 설정
...
{
    var thisObj = this, item, aview, newItems = [], cssKey;
	...
	for(var i=0; i<dataArray.length; i++)
	{
		...
		Object.assign(item.style, cssObj);

		if(isPrepend) this.element.prepend(item);
		else this.element.append(item);
...
{
	return Array.prototype.indexOf.call(this.getItems(), item);
};
...
ASlideView.prototype.slideTo = function(index, isReport)
{
	...
	if(this.option.direction=='vertical') targetVal = this.getHeight()*index;
	else targetVal = this.getWidth()*index;

	var prop = (this.option.direction=='vertical') ? 'scrollTop' : 'scrollLeft';
	var startVal = this.element[prop];
	var distance = targetVal - startVal;
	var duration = this.option.moveSpeed;
	var startTime = null;

	if(this._slideAnim) cancelAnimationFrame(this._slideAnim);
	...
	function step(timestamp)
	{
		...
		self.element[prop] = startVal + distance * progress;
		if(progress < 1) self._slideAnim = requestAnimationFrame(step);
		else self._slideAnim = null;
	}
	this._slideAnim = requestAnimationFrame(step);
...
{
	if(this.inx+1==this.element.children.length) this.inx--;
...
ASlideView.prototype.updatePosition = function(pWidth, pHeight)
{
	...
	if(this.option.direction=='vertical') this.element.scrollTop = this.getHeight()*this.inx;
   	...
   	for(const child of this.getItems())
   	{
       	if(child.view) child.view.updatePosition();
   	}
};
