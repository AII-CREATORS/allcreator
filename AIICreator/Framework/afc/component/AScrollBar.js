...
AScrollBar.prototype.init = function(context, evtListener)
{
	...
	this.cntLo = children[0];
	this.cntHi = children[1];
...
AScrollBar.prototype.setScrollType = function(type)
{
	...
	if(this.isScrollVert)
	{
		...
		this.cntLo.style.width = '100%';
		this.cntHi.style.width = '100%';

		this.cntLo.style.height = '0px';
		this.cntHi.style.height = '0px';
	...
	else
	{
		...
		this.cntLo.style.height = '100%';
		this.cntHi.style.height = '100%';

		this.cntLo.style.width = '0px';
		this.cntHi.style.width = '0px';
...
AScrollBar.prototype.setScrollArea = function(scrlAreaHeight, scrollPadding, scrollGap)
{
    ...
    //데이터가 먼저 세팅되고 호출되면 스크롤표시가 되지 않아서 호출해줌
    if(this.dataCount) this.setDataCount(this.dataCount);
};
...
AScrollBar.prototype.setDataCount = function(dataCount)
{
	...
	if(this.isScrollVert)
	{
		this.cntLo.style.height = half + 'px';
		this.cntHi.style.height = (half+mod) + 'px';
	...
	{
		this.cntLo.style.width = half + 'px';
		this.cntHi.style.width = (half+mod) + 'px';
	}
...
AScrollBar.prototype.addWheelArea = function(wheelArea)
{
	...
	wheelArea.addEventListener("wheel", _wheelHandler, false);
    ...
    if(!this._wheelAreaHandlerMap) this._wheelAreaHandlerMap = new Map();
    this._wheelAreaHandlerMap.set(wheelArea, _wheelHandler);
};

AScrollBar.prototype.removeWheelArea = function(wheelArea)
{
    if(this._wheelAreaHandlerMap.get(wheelArea))
    {
        wheelArea.removeEventListener('wheel', this._wheelAreaHandlerMap.get(wheelArea));
        this._wheelAreaHandlerMap.delete(wheelArea);
    }
};
...
AScrollBar.prototype.scrollToCenter = function()
{
