...
DnDManager.prototype.unregDrag = function(element)
{
	...
	else _unreg_drag(element);

	function _unreg_drag(ele)
	{
		ele.removeAttribute('draggable');
		//dragstart, dragend 이벤트는 addEventListener 로 등록되어 있으므로
		//개별 핸들러 참조 없이는 제거할 수 없음
	}
...
DnDManager.prototype.unregDrop = function(element)
{
	...
	else _unreg_drop(element);

	function _unreg_drop(ele)
	{
		//dragenter, dragleave, dragover, drop 이벤트는 addEventListener 로 등록되어 있으므로
		//개별 핸들러 참조 없이는 제거할 수 없음
	}
};
...
DnDManager.prototype.regDrop = function(element, listener, option)
{
	...
	function _apply_hoverClass(evt, isAdd)
	{
		...
		if(hoverClass)
		{
			...
			if(isAdd) targetEle.classList.add(hoverClass);
			else targetEle.classList.remove(hoverClass);
		}
	}
	...
	function _drop_helper(ele)
	{
		ele.addEventListener('dragenter', function(e)
		{
			...
			{
				var hoverClass = thisObj.getApplyDropOption(option, 'hoverClass');
				if(hoverClass) e.target.classList.add(hoverClass);
		...
		ele.addEventListener('dragleave', function(e)
		{
			...
			{
				var hoverClass = thisObj.getApplyDropOption(option, 'hoverClass');
				if(hoverClass) e.target.classList.remove(hoverClass);
