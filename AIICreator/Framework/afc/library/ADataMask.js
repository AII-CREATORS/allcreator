...
ADataMask.Number = 
{
	...
	multiply:
	{
		title: '원하는 값으로 곱한다',
		param: ['곱할값', '정수여부'],
		func: function divide(value, param, ele)
		{
			value = value * (param[0]||1);
			if(param[1]) value = parseInt(value);
			return value;
		}
	},
	// 나누기
...
ADataMask.Date = 
{
	...
	formatTime:
	{
		title: '시간 숫자값을 포맷 형태로 변경한다',
		param: ['포맷 yyyyMMddHHmmssms'],
		func: function formatByTime(value, param, ele)
		{
			if(!param[0]) return value;
			return new Date(value).format(param[0]);
		}
	}
};
...
ADataMask.DataGrid = 
{
    dataType:
    {
        ...
        func: function dataType(value, param, ele, dataObj)
        {
...
ADataMask.Text = 
{
	prefix:
	{
		...
		func: function prefix(value, param, ele)
		{
			var txt = '';
	...
	suffix:
	{
		...
		param: ['들어갈 문자', '값 있을때만 문자추가(생략시 false)'],
		func: function suffix(value, param, ele)
		{
			var txt = '';
			if(param[0]) txt = param[0];
            if(param[1]&&!value) txt = '';
	...
	substring:
	{
		title: '문자열의 부분 문자열을 반환한다.',
		param: ['시작 인덱스', '끝 인덱스(생략 가능)'],
		func: function substring(value, param, ele)
		{
			if(param[0]) value = value.toString().substring(param[0], param[1]);
			return value;
		}
	},
	toUpperCase:
	{
		title: '문자열을 대문자로 변환해 반환한다.',
		param: [],
		func: function toUpperCase(value, param, ele)
		{
	...
	toLowerCase:
	{
		title: '문자열을 소문자로 변환해 반환한다.',
		param: [],
		func: function toLowerCase(value, param, ele)
		{
	...
	padStart:
	{
		title: '결과 문자열이 주어진 길이에 도달할 때까지 시작 부분에 다른 문자열을 채운다.',
		param: ['총 길이', '채울 문자열'],
		func: function padStart(value, param, ele)
		{
			if(param[0]) return value.toString().padStart(param[0], param[1]);
			return value;
		}
	},
	padEnd:
	{
		title: '결과 문자열이 주어진 길이에 도달할 때까지 끝 부분에 다른 문자열을 채운다.',
		param: ['총 길이', '채울 문자열'],
		func: function padEnd(value, param, ele)
		{
			if(param[0]) return value.toString().padEnd(param[0], param[1]);
			return value;
		}
	}
};
